...
- Reject DSL v4 HTTP surfaces that use `CustomAuth` or send credentials to non-HTTPS, non-loopback provider URLs. Coral enforces this at runtime for top-level auth, identity-produced headers, and authored request headers; query parameters and bodies must not be used to bypass the rule. Loopback HTTP means exact `localhost`, IPv4 loopback `127.0.0.0/8`, or IPv6 loopback `[::1]`. Reject redirect-dependent v4 endpoints and require canonical URLs.

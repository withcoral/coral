...
- For DSL v4 surfaces, `identity_requirements` replaces source-stored credentials when installed source config will bind the surface to an existing identity. Verify the source or docs make that identity dependency clear and do not also model duplicate source-token setup for the same auth path.
- If a source file includes identity specs, it should be a YAML stream with exactly one source document plus `kind: identity` documents separated by `---`. `coral source add --file` imports those specs, but they are global app state; they must not be described as creating identity records or source bindings.
- If an OAuth identity spec declares `inputs`, verify client IDs use variable inputs or safe spec defaults, client secrets reference `kind: secret` inputs, and OAuth endpoint templates reference only declared `kind: variable` inputs. Secret identity inputs are stored as installed identity-spec input material for refresh; they must not be modeled as variables, source inputs, or concrete identity material. Secret input values must be trimmed before storage, and whitespace-only values must be rejected as missing.

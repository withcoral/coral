...
- For DSL v4 surfaces, `identity_requirements` is used only when installed source config will bind the surface to an existing user-owned or workspace-owned identity. It must not be treated as a replacement for source-stored credentials unless the source intentionally depends on those workspace bindings; the source or docs should make that dependency clear.
- If a source file bundles identity specs, it should be a YAML stream with exactly one source document plus `kind: identity` documents separated by `---`. Identity specs are global app state; bundled specs must not be described as creating identity records or source bindings.
- If an OAuth identity spec declares `inputs`, verify client IDs use variable inputs or safe spec defaults, client secrets reference `kind: secret` inputs, and OAuth endpoint templates reference only declared `kind: variable` inputs. Secret identity inputs are stored as installed identity-spec input material for refresh; they must not be modeled as variables, source inputs, or concrete identity material.
- If a DSL v4 surface declares `identity_requirements`, each accepted entry has a stable `id`, non-empty `identity_specs`, and appropriate audience scoping. Verify the referenced `kind: identity` specs carry the correct provider `issuer`, identity `type`, setup method, and least-privilege capabilities that match the provider docs. For built-in HTTP identity specs, `audience.host` must be a string because Coral injects Authorization headers only for that exact host or its subdomains. Flag missing or overbroad permissions, missing binding docs, or identity requirements on sources meant to work with ordinary `coral source add --file` local credential setup.

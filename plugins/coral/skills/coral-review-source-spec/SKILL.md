...
- Reject DSL v4 HTTP surfaces that use `CustomAuth` or send credentials to non-HTTPS, non-loopback provider URLs. Coral enforces this at runtime for top-level auth and identity-produced headers; ordinary request headers, query parameters, and bodies must not be used to bypass the rule. Loopback HTTP means `localhost`, `127.0.0.1`, or `[::1]`.

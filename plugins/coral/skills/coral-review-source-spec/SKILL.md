...
- If a secret declares `credential.methods`, each method matches the provider's supported setup path. OAuth methods use authorization-code flow, an explicit `pkce` value, loopback redirect URI, correct endpoint URLs, appropriate client ID/default/input behavior, required `client.id.input` when the token endpoint needs client-secret authentication, correct client-secret transport when needed, and least-privilege scopes. Authorization-code OAuth can support SSH, VM, and split-browser setups because the CLI accepts a pasted final localhost redirect URL while waiting for the loopback callback.

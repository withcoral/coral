...
- When `from: one_of` models alternate stored credentials, mark each branch's secret input `required: false` so either credential can satisfy auth. Coral requires at least one of those secrets before the source can be added when every choice references a secret input.

...
- Installing or importing an identity spec only installs the spec. It does not create identity records or source bindings. Its `inputs` are collected at spec install and stored with the installed spec, not with a concrete identity. Trim secret input values before storage and reject whitespace-only values as missing.

...
- OAuth endpoints must use HTTPS or explicit parsed loopback HTTP, contain no userinfo or fragments, and be canonical because Coral's OAuth HTTP client does not follow redirects. Coral validates each browser URL before handoff; subsequent browser navigation is outside Coral's outbound policy.

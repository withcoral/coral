...
- Use `from: bearer` for raw access tokens that must be sent as `Authorization: Bearer <token>`.
- Use `from: one_of` only when a provider really has separate stored credential slots or different runtime auth locations. Mark each branch's secret input `required: false` so either credential can satisfy auth. Coral requires at least one referenced secret before the source can be added unless another choice always supplies a value on its own, such as a non-empty literal.

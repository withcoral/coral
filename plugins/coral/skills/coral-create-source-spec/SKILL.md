...
- When the provider supports OAuth Dynamic Client Registration, prefer `client.dynamic_registration.registration_url` over hard-coding a shared public client ID. Set `token_endpoint_auth_method` to `none`, `client_secret_basic`, or `client_secret_post` to match the registered client type. Use `initial_access_token_input` only when the registration endpoint requires a protected initial access token. Set `request_refresh_token_grant: true` only when the provider supports registering clients for refresh-token grants. For OAuth-protected Streamable HTTP MCP servers, set `oauth.resource` to the MCP server URL when the provider requires resource indicators.

...
- For DSL v4 HTTP surfaces, do not use `CustomAuth`; use identity requirements instead. Top-level auth, identity-produced headers, and authored request headers require HTTPS or loopback HTTP (exact `localhost`, IPv4 loopback `127.0.0.0/8`, or IPv6 loopback `[::1]`). DSL v4 HTTP requests do not follow redirects, so use canonical endpoint URLs. Never move credentials into query parameters or bodies to bypass this rule.
...
- OAuth endpoints must use HTTPS or explicit parsed loopback HTTP, contain no userinfo or fragments, and be canonical because Coral's OAuth HTTP client does not follow redirects. Coral validates rendered endpoints, provider-returned verification URLs, and stored refresh endpoints. Coral validates each browser URL before handoff; subsequent browser navigation is outside Coral's outbound policy.

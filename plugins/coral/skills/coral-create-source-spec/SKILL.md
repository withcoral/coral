...
- OAuth provider endpoints (`authorization_url`, `device_authorization_url`, and `token_url`) must use `https`, except for explicit loopback `http` endpoints used for local development or tests. OAuth endpoint URLs may template declared `kind: variable` inputs with `{{input.KEY}}` for non-secret endpoint components such as tenant IDs or domains. Do not reference secret inputs, filters, function arguments, state, or inline defaults from OAuth endpoint URLs.

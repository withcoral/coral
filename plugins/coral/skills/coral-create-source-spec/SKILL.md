...
- In DSL v3, author Universal Search policy on the table function with `universal_search.id`, `execute`, and, for an allow, `query_arg`. Make every route ID match `[a-z][a-z0-9_]*` and remain unique within the source. The allowed query argument must have type `Utf8` and no default; every other exposed argument needs a type-correct default.
...
- Keep result mapping separate from authorization. DSL v3 mappings name authored result columns. DSL v4 mappings use RFC 6901 JSON Pointers to imported result fields. Prefer ordered stable identity fields and map only the provider ID, title, URL, snippet, and attributes needed for a useful result.
...
Read `references/http-source-checklist.md` when you need table-shape,
Universal Search authorization, or pagination guidance.

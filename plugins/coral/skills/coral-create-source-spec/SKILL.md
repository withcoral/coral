...
- For DSL v4 HTTP surfaces, do not use `CustomAuth`; use identity requirements instead. Top-level auth, identity-produced headers, and authored request headers require HTTPS or loopback HTTP (`localhost`, `127.0.0.1`, or `[::1]`). DSL v4 HTTP requests do not follow redirects, so use canonical endpoint URLs. Never move credentials into query parameters or bodies to bypass this rule.

...
- For DSL v4 HTTP surfaces, do not use `CustomAuth`; use identity requirements instead. Top-level auth and identity-produced headers require HTTPS or loopback HTTP (`localhost`, `127.0.0.1`, or `[::1]`). Never move credentials into ordinary request headers, query parameters, or bodies to bypass this rule.

  ...
  it('preserves the hosted callback redirect and session cookie response', async () => {
    ...
    const callbackResponse = new Response(null, {
      headers: [
        ...
        ['Set-Cookie', 'coral_ui_oauth=; Max-Age=0; Path=/; HttpOnly'],
        ['Set-Cookie', 'coral_ui_session=encrypted; Path=/; HttpOnly; Secure'],
      ],
      status: 302,
    })
    authMocks.completeCoralOAuthLogin.mockResolvedValue(callbackResponse)
    const request = new Request('https://coral-ui.example.test/auth/callback?code=code&state=state')
...
async function runLoader() {
  return loader({
    request: new Request('https://coral-ui.example.test/auth/callback?code=code&state=state'),
  } as never)
}

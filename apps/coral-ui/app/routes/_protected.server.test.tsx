...
const authMocks = vi.hoisted(() => ({
  clearCoralUISession: vi.fn(async () => 'coral_ui_session=; Max-Age=0; Path=/; HttpOnly'),
...
describe('optional auth boundary', () => {
  beforeEach(() => {
    authMocks.clearCoralUISession.mockClear()
  ...
  it.each([
    ['document', 'GET', 'https://coral-ui.example.test/workspaces/analytics/sources'],
    ['data', 'GET', 'https://coral-ui.example.test/workspaces/analytics/sources.data?index'],
    ['action', 'POST', 'https://coral-ui.example.test/workspaces'],
    ['resource action', 'POST', 'https://coral-ui.example.test/sources/github/oauth-install'],
  ])(
  ...
  it('clears a Coral-rejected session through the real React Router middleware pipeline', async () => {
    ...
    const response = await handleRequest(
      new Request('https://coral-ui.example.test/protected?tab=data'),
    )
    ...
    expect(authMocks.clearCoralUISession).toHaveBeenCalledOnce()
  ...
  it('converts a Coral-rejected stream action in the real middleware pipeline', async () => {
    ...
    const response = await handleRequest(
      new Request('https://coral-ui.example.test/protected', {
        headers: {
  ...
  it('navigates when the OAuth action expires inside the protected route pipeline', async () => {
    ...
    const fetchOAuthInstall = vi.fn<typeof fetch>(async (input, init) => {
      ...
      serverResponse = await handleRequest(
        new Request(new URL(inputUrl, 'https://coral-ui.example.test'), init),
      )
  ...
  it.each([
    ...
    async (_, method, pathname) => {
      ...
      const response = await handleRequest(
        new Request(`https://coral-ui.example.test${pathname}`, { method }),
      )
  ...
  it('clears an unreadable session cookie on the login redirect', async () => {
    ...
    const thrown = await runMiddleware(
      new Request('https://coral-ui.example.test/', {
        headers: { cookie: 'other=value; coral_ui_session=unreadable' },
      }),
      vi.fn(),
  ...
  it('keeps the server-held access token out of the serialized route response', async () => {
    ...
    const response = await handleRequest(
      new Request('https://coral-ui.example.test/workspaces/analytics/sources/github.data'),
    )
  ...
  it('commits a fresh CSRF cookie on the protected response', async () => {
    ...
    authMocks.csrfTokenForRequest.mockResolvedValue({
      setCookie: 'coral_ui_session_csrf=signed; Path=/; HttpOnly; SameSite=Lax',
    ...
    expect(response.headers.get('Set-Cookie')).toContain('coral_ui_session_csrf=signed')

    const childError = new Response('failed', { status: 500 })
    const thrown = await runMiddleware(new Request('https://coral-ui.example.test/'), async () => {
      throw childError
    }).catch((error: unknown) => error)
    expect((thrown as Response).headers.get('Set-Cookie')).toContain('coral_ui_session_csrf=signed')
  })
  ...
  it('uses the same client-visible signal when the encrypted session is already expired', async () => {
    ...
    const thrown = await runMiddleware(
      new Request('https://coral-ui.example.test/workspaces/analytics/sources/oauth-import', {
        headers: {
          cookie: 'coral_ui_session=expired',
          [AUTH_STREAM_REQUEST_HEADER]: '1',
          [AUTH_STREAM_RETURN_TO_HEADER]: '/workspaces/analytics/sources/new',
        },
        method: 'POST',

...
const discoverRequest = (url: string, signal?: AbortSignal) =>
  new Request(
    `http://coral-ui.test/workspaces/default/sources/discover?url=${encodeURIComponent(url)}`,
...
`),
  ...
  it('loads the URL and returns a query-safe source name', async () => {
    ...
    const request = new Request(
      'http://coral-ui.test/workspaces/default/sources/discover?url=https%3A%2F%2Fstatus.example%2Fopenapi.json',
    )
  ...
  it('resolves a relative server URL against the document location', async () => {
    ...
    const request = new Request(
      'http://coral-ui.test/workspaces/default/sources/discover?url=https%3A%2F%2Fstatus.example%2Fdocs%2Fopenapi.json',
    )
  ...
  it('rejects non-HTTPS discovery URLs without fetching them', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    const request = new Request(
      'http://coral-ui.test/workspaces/default/sources/discover?url=http%3A%2F%2Flocalhost%2Fopenapi.json',
    )
  ...
  it('detects an HTTP-rejecting /mcp endpoint as an MCP server', async () => {
    ...
    const request = new Request(
      'http://coral-ui.test/workspaces/default/sources/discover?url=https%3A%2F%2Ftools.example%2Fmcp',
    )
  ...
  it('detects an /sse endpoint with a trailing slash and query as an MCP server', async () => {
    ...
    const request = new Request(
      'http://coral-ui.test/workspaces/default/sources/discover?url=https%3A%2F%2Ftools.example%2Fevents%2Fsse%2F%3Ftoken%3Dsecret',
    )

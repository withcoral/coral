...
import { oauthCookieName, readOAuthTransaction, readCoralUISession } from './session.server'
...
describe('Coral OAuth adapter', () => {
  ...
  it('starts login with PKCE and identity derived only from the Coral UI public URL', async () => {
  ...
  it('completes login, clears OAuth state, and stores a bounded encrypted session', async () => {
    ...
    const login = await startCoralOAuthLogin(
      new Request('https://coral-ui.example.test/login?returnTo=/workspaces/analytics?tab=mine'),
      config,
    )
    ...
    const session = await readCoralUISession(
      new Request('https://coral-ui.example.test/', {
        headers: { cookie: cookieHeader(callback, 'coral_ui_session') },
      }),
      config,
    )
    ...
    expect(cookies[1]).toContain('coral_ui_session=')
  ...
  it.each([
    ...
    const login = await startCoralOAuthLogin(
      new Request(`https://coral-ui.example.test/login?returnTo=${encodeURIComponent(returnTo)}`),
      config,
    )
    const state = new URL(login.headers.get('location') ?? '').searchParams.get('state')
    const callback = await completeCoralOAuthLogin(
      new Request(`https://coral-ui.example.test/auth/callback?code=abc123&state=${state}`, {
        headers: { cookie: cookieHeader(login, oauthCookieName(config)) },
      }),
      config,
    )
  ...
  it('clears OAuth state only after a callback matches the active transaction', async () => {
    const fetch = mockFetch(jsonResponse(metadata))
    const login = await startCoralOAuthLogin(
      new Request('https://coral-ui.example.test/login'),
      config,
    )
    ...
    const providerError = await completeCoralOAuthLogin(
      new Request(
        `https://coral-ui.example.test/auth/callback?error=access_denied&state=${state}`,
        {
          headers: { cookie },
        },
      ),
    ...
    const stateError = await completeCoralOAuthLogin(
      new Request('https://coral-ui.example.test/auth/callback?code=abc&state=wrong', {
        headers: { cookie },
      }),
      config,
    ).catch((error: unknown) => error)
  ...
  it('normalizes direct token-endpoint OAuth JSON into the callback error contract', async () => {
    ...
    const error = await completeCoralOAuthLogin(
      new Request(`https://coral-ui.example.test/auth/callback?code=abc&state=${state}`, {
        headers: { cookie: cookieHeader(login, oauthCookieName(config)) },
      }),
      config,
  ...
  it('uses RFC 8414 discovery for a path issuer and requires its exact identifier', async () => {
    ...
    await startCoralOAuthLogin(new Request('https://coral-ui.example.test/login'), pathConfig)
  ...
  it('rejects metadata for a different issuer', async () => {
    ...
    await expect(
      startCoralOAuthLogin(new Request('https://coral-ui.example.test/login'), config),

...
describe('Coral endpoint policy', () => {
  ...
  it('accepts authenticated HTTPS and ignores an irrelevant opt-in value', () => {
    expect(
      resolve({
        ...
        CORAL_UI_ALLOW_INSECURE_CORAL_ENDPOINT: 'garbage',
      }),
  ...
  it.each([' 1 ', ' TrUe '])('accepts normalized cleartext opt-in %j', (optIn) => {
    expect(
      resolve({
        ...
        CORAL_UI_ALLOW_INSECURE_CORAL_ENDPOINT: optIn,
      }),
    ).toEqual({
  ...
  it('rejects localhost subdomains and validates garbage only on an insecure path', () => {
    ...
    expect(() =>
      resolve({
        CORAL_ENDPOINT: 'http://coral.internal:14555',
        CORAL_UI_ALLOW_INSECURE_CORAL_ENDPOINT: 'yes',
      }),
    ).toThrow('CORAL_UI_ALLOW_INSECURE_CORAL_ENDPOINT must be set to 1 or true')
  })
  ...
  it.each(['file:///tmp/coral.sock', 'grpc://coral.internal:14555'])(
    ...
    (CORAL_ENDPOINT) => {
      expect(() =>
        resolve({ CORAL_ENDPOINT, CORAL_UI_ALLOW_INSECURE_CORAL_ENDPOINT: 'true' }),
      ).toThrow('CORAL_ENDPOINT must be an absolute HTTP(S) URL')
    },
  )

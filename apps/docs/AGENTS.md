...
- Before adding new text, search for existing coverage, choose the best
  ...
  paragraphs.
- `apps/docs/reference/bundled-sources.mdx` and
  `apps/docs/reference/community-sources.mdx` are generated by
...
- `make docs-check` intentionally skips
  `apps/docs/reference/community-sources.mdx`; any PR may leave that generated page
...
- The live docs site deploys from the long-lived `docs` branch, not `main`.
  ...
  binary cannot run.
- `main` still owns docs freshness: keep `apps/docs/` aligned with the code as today,

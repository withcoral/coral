...
Use the reference on the first line with `coral sql` to fetch rows. See
[Query your data](/getting-started/quickstart#3-query-your-data) for a complete
example.

Here, `github` is the query-visible SQL schema; JSON and MCP results return it as `schema_name`. For a source with multiple runtime components, it can differ from the installed source name because each component can publish its own SQL schema.

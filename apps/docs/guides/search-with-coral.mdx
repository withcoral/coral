...
DSL v3 results use two-part `schema.relation` references. Database and DSL v4
results use three-part `catalog.schema.relation` references. For DSL v4, the
catalog is the source name and the schema comes from the operation's first tag,
normalized as a SQL identifier, or is `public` when no tag exists. For example,
a tagged v4 function can render as
`github_v4.issues.issues_list_for_repo(...)`. Use the returned reference instead
...
Here, `github` is the query-visible SQL schema. JSON and MCP catalog results
return `catalog_name` and `schema_name` separately; an empty `catalog_name`
means the reference is two-part. For a source with multiple runtime components,
the query-visible identity can differ from the installed source name. Use the
complete returned identity with the result's table or table function and column
when you write SQL.

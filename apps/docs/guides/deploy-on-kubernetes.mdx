...
`coral.example.com` (console), `auth.coral.example.com` (OAuth issuer), and
...
without Coral's approval page, since the console's client ID is listed in
`trusted_clients`.
...
Day-to-day administration happens in the console. For the rest, run the CLI

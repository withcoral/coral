...
## Shared MCP serves one configured workspace

An MCP HTTP listener with login enabled serves exactly one workspace, named in
configuration:
...
`workspace` is required for this surface to serve anybody, and `public_url` is
required whenever login is enabled. The name is the operator's selection for the
listener as a whole: it is not per-caller, and there is no default to fall back
on. A listener that names nothing refuses every session rather than resolving a
workspace on somebody's behalf.

Startup validates that the name is a syntactically valid workspace name, and
does not check that the workspace exists. Existence is a per-caller question, so
it is answered per caller, at connect time — which also means you can name a
workspace before creating it.
...
the configured name is exactly one of them. No workspace is substituted and none
...
```
Workspace `analytics` was not found. Name a workspace you can access in
`server.mcp_http.workspace`, or create one in Reef.
```
...
If the listener names no workspace at all, callers get a different message, and
this one is about your configuration rather than about any workspace:

```
This MCP server has no workspace configured. Create a workspace in Reef, then
name one you can access in `server.mcp_http.workspace`.
```

...
The local stack runs Coral, Coral UI with authentication disabled
(`CORAL_UI_AUTH_MODE=disabled`), and Envoy in between: with authentication
disabled, Coral UI uses a gRPC-Web transport that `coral server` does not
speak, so Envoy translates it to native gRPC. Everything is published only on
loopback.

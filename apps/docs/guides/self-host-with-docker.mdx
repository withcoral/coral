...
configured and Coral UI runs with `CORAL_UI_AUTH_MODE=disabled`. In that
setup, Coral UI uses a gRPC-Web transport that `coral server` does not speak,
so Envoy sits between the two translating it to native gRPC. Everything is

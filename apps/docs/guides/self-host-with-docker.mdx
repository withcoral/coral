...
identity provider's login and land back in the console signed in. The
`trusted_clients` entry in the seed config is the console's own client ID, so
console sign-ins skip Coral's approval page; drop that line to have every
sign-in pause on the approval page instead (see
...
unlike the console, MCP clients are not listed in `trusted_clients` — before

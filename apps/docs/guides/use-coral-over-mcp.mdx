...
Coral ships with a built-in MCP server that presents Coral to agents as a read-only SQL database with catalog and observed-value search.
...
When the right table or table function is not obvious, ask the agent to [search with Coral](/guides/search-with-coral). The `search` tool returns catalog metadata and, when observed-value search is enabled, locally observed values. An observed-value match is a local routing clue that can help the agent locate the right table and column; it does not fetch current source rows or prove that the value is still present in a connected source.

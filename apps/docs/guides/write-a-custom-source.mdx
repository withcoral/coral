...
- **HTTP credentials Coral can store locally** — an API key, personal access token, service account credential, or OAuth flow that returns an access token for Coral to use on API requests
...
OAuth-backed APIs are good candidates when their setup and token lifecycle fit one of Coral's supported flows. During `coral source add --interactive`, Coral obtains an access token and stores it as a source secret. See [OAuth flow and client settings](/reference/source-spec-reference#oauth-flow-and-client-settings) for flow-specific requirements and token renewal behavior.
...
When a source should offer OAuth setup, add `credential.methods` to the secret input while keeping runtime `auth` focused on how the stored secret is sent to the API. Use `type: oauth` for OAuth setup, and add a `type: source_config` fallback when users can paste an existing token.

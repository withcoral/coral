...
session signing key, and the upstream identity provider. The MCP
listener's public URL becomes its protected-resource identifier and token
audience:
...
resolved relative to Coral's config directory. MCP accepts only tokens whose
audience is its canonical public URL. The private gRPC API accepts the explicit
first-party audience allowlist, which currently contains that same MCP URL;
gRPC health checks remain unauthenticated. The explicit insecure-bind
acknowledgement is required because Coral serves cleartext HTTP on that socket.
Put it behind TLS termination and prevent untrusted clients from reaching the
private gRPC listener directly. While MCP HTTP and gRPC run in one process
started by `coral server`, the gRPC bind must be loopback or wildcard so MCP can
forward the bearer over a loopback connection.

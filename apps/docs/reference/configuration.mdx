...
| `public_url`                         | string           | unset         | Externally reachable identifier for the MCP endpoint. Required with `[auth]`, forbidden without it.   |
...
With `[auth]`, `bind` may be any address and `public_url` is required; its
value is the OAuth resource identifier and token audience for the endpoint, so

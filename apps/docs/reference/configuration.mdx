...
re-authenticate within. A public surface accepts only tokens whose audience is its
own canonical `public_url`, so a token minted for one surface cannot be replayed
at another.

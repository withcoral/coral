...
the instance's canonical identifier.
...
and point the TLS terminator for that hostname at it. Serving these endpoints
cleartext outside loopback additionally requires
`auth.allow_insecure_remote_http_bind = true`.

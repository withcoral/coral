...
`add_function` accepts `schema`, `name`, `description`, and one read-only SQL query. Values such as `$owner` in the query become required named arguments. It refuses to overwrite an existing function unless the agent passes `replace_existing: true`, and Coral validates a replacement before changing the installed function.

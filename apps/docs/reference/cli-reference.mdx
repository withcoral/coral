...
workspace at its own URL, `/mcp/workspace/{workspace}`. Which workspace an MCP
session reaches is decided by the URL the client connects to — never by
...
Without `[auth]` the listener is loopback-only and unauthenticated: requests

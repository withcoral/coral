...
Create or replace a current-user identity from a global fixed-token identity
spec. In a terminal, Coral prompts for the token without echoing it.
...
```shellscript
printf '%s\n' "$GITHUB_TOKEN" | coral identity add github_local --identity-spec github_token --token-stdin
```

Token setup material is write-only. Coral encrypts it for storage and never
returns it from identity management APIs or renders it in command output.

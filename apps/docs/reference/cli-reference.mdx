...
`coral.table_functions`, `coral.inputs`, and `coral.task_queries`.

`coral.task_queries` contains retained MCP SQL activity for tasks in the current
workspace. Filter by `task_id`, then order by `executed_at, query_id` for a
deterministic execution-start order.

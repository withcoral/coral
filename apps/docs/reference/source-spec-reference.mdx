...
Tables support an optional `guide` field for query advice that does not belong
in the short `description`, such as when to prefer another resource or which
query pattern avoids a provider limit.

Set `require_guide_read: true` when an MCP agent must see that guide before it
first uses the table in a task:
...
tables:
  - name: channels
    ...
    guide: |
      Do not use this table for lookup by name or ID.
      Use user_channels for lookup queries.
    require_guide_read: true
```

Coral blocks the first MCP `sql` call in the task that references the table,
returns the guide without executing any query in that call, and unblocks that
guide version for the rest of the task. If the guide changes, Coral blocks the
next MCP `sql` call for that table and returns the updated guide. The agent can
retry unchanged after following the guide or submit a revised query. A required
guide must not be empty. Non-MCP SQL clients are unchanged.
...
Table functions support the same optional `guide` and
`require_guide_read` fields as tables, with the same per-task MCP behavior.
Keep `description` short and use `guide` for usage advice, such as when to
prefer the function, how to prepare its arguments, or provider-specific query
syntax.

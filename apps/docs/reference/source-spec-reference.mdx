...
Coral attaches top-level auth and identity-produced headers only over HTTPS or
loopback HTTP (`localhost`, `127.0.0.1`, or `[::1]`); other plaintext targets
fail closed. Do not move credentials into ordinary request headers, query
parameters, or bodies to bypass this rule.

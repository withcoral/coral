...
`require_guide_read` requires a non-empty guide. Coral blocks the first matching
MCP `sql` call without executing it; the agent can then retry or revise the
query. Other SQL clients are unaffected.

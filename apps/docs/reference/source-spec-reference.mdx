...
- `oauth`: run an OAuth device-code, authorization-code, or client-credentials
  flow and store the returned access token in the same source secret.

Use `type: oauth` when the provider should issue the source secret through an
OAuth setup instead of asking the user to paste a token. If you want to support
both setup paths, list the OAuth method first and add a `source_config` fallback
...
OAuth supports device-code flow, authorization-code flow, and
...
For client-credentials flow, declare `flow.type: client_credentials`,
`endpoints.token_url`, `client.id.input`, `client.secret.input`, and a
`client.secret.transport` of `basic_auth` or `request_body`. Client-credentials
OAuth does not use user authorization, redirect URI fields, authorization or
device-authorization endpoints, PKCE, or Dynamic Client Registration. Coral
exchanges the client credentials for an access token during source setup and
stores the client metadata needed to re-mint the access token when the token
response includes usable `expires_in` metadata. Without that expiry signal,
users must reconnect after the provider stops accepting the token.

Authorization-code and device-code methods can also declare
`client.dynamic_registration` with a `registration_url`. When no static client
ID is provided, Coral dynamically registers a client during interactive
credential setup and uses the returned client ID, optional client secret, and
token endpoint auth method for the OAuth flow. Coral always sends
`application_type: native` during Dynamic Client Registration because these
OAuth setup paths use native-app flows: loopback authorization-code redirects
or device code. Source specs cannot configure a `web` Dynamic Client
Registration application type. Set
...
same resource value on the requests relevant to the selected flow, including
client-credentials token minting and re-minting.
...
metadata alongside the source secret, preserves rotated refresh tokens when
the provider returns a new one during refresh, and reuses declared scopes when
re-minting client-credentials access tokens.

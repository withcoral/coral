...
operation. The selected query input must lower to an exposed `Utf8` argument
with no default. Every other exposed input must have a type-compatible default
in the imported operation; these defaults are imported surface facts, not
fields authored in the DSL v4 manifest.
...
- Any authored DSL v4 `universal_search` policy puts the source in explicit mode;
  unlisted routes are not inferred.
- Without authored policy, only one unambiguous, safe generated search route
  can be inferred.
- Duplicate or missing targets, ambiguous query inputs, missing or
  type-incompatible imported defaults, and unresolved result fields do not fall
  back to a guessed operation.

...
DSL v4 surfaces must not use `CustomAuth`; use identity requirements instead.
Coral attaches top-level auth, identity-produced headers, and authored request
headers only over HTTPS or loopback HTTP (exact `localhost`, IPv4 loopback
`127.0.0.0/8`, or IPv6 loopback `[::1]`);
other plaintext targets fail closed. DSL v4 HTTP requests do not follow
redirects, so declare canonical endpoint URLs. Do not move credentials into
query parameters or bodies to bypass this rule.
...
OAuth endpoint URLs must use HTTPS, except that HTTP is allowed for exact
`localhost` or a parsed IPv4 or IPv6 loopback address. Coral applies this after
template rendering and to provider-returned verification URLs and persisted
token endpoint overrides used for refresh. URLs with userinfo or fragments are
rejected. Coral's OAuth HTTP client does not follow redirects, so configure
canonical endpoint URLs. Coral validates each authorization or verification URL
before handing it to the CLI; subsequent browser navigation is outside Coral's
...
Authorization-code flow supports loopback HTTP callback URIs using exact
`localhost`, an IPv4 loopback address in `127.0.0.0/8`, or IPv6 loopback
`[::1]`. Callback URIs must not contain userinfo or fragments.

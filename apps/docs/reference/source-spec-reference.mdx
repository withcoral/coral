...
Table functions support the same optional `guide` field as tables. Keep
`description` short and use `guide` for usage advice, such as when to prefer
the function, how to prepare its arguments, or provider-specific query syntax.

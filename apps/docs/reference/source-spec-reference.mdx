...
<Tip>
  ...
  source](/guides/write-a-custom-source). Use this page for field-level details,
  including HTTP authentication, source inputs, OAuth credential methods,
...
| `version`      | DSL v3 source spec version string                                                |
| `dsl_version`  | Coral source spec DSL version: `3`, or `4` for a preview imported-surface source |
| `backend`      | DSL v3 fetch backend: `http`, `file`, or `mcp`                                   |
| `surface`      | DSL v4 imported OpenAPI or MCP surface                                            |
...
- Each query result records the SQL text, pass/fail status, and either a row count
  or an error message.
...
When authoring source specs, prefer the `parent__child` style for flattened
nested fields so related values are recognizable in SQL and metadata tables.
...
Use `guide` for query instructions that are too detailed for `description`.
Set `require_guide_read: true` to show the guide before an MCP agent first
queries the table in each task:

```yaml
tables:
  - name: channels
    description: Slack channels
    guide: Prefer user_channels for lookup by name or ID.
    require_guide_read: true
```

`require_guide_read` requires a non-empty guide. Coral returns the guide instead
of executing the first matching query in each MCP task; other queries in the
same batch still run. The agent can then retry or revise the query. Other SQL
clients are unaffected. In a DSL v4 projection override, set the same fields on
the projection.
...
rows and need invocation arguments. A function adds invocation arguments, but it
still owns the same execution shape as an HTTP table: request, response mapping,
pagination, and result columns. It does not need a backing table.

Use the default `kind: table` for parameterized non-retrieval operations, such as
scoped child collections, time-range log queries, metrics queries, or detail
operations that do not map cleanly to a stable table. If the endpoint is a
...
Function args also accept an optional typed `default`. Coral preserves the
YAML/JSON value type rather than converting every default to text. The default
must match the argument `type` and any declared `values`.
...
functions:
  - name: search_issues
    ...
    guide: |
      Use this function to find issues by keywords or qualifiers.
      Prefer it over scanning the issues table for lookup queries.
    search_limits:
      default_top_k: 10
      max_top_k: 100
...
Table functions support `guide` and `require_guide_read` with the same behavior.
...
Search result columns should help users and agents decide which candidate to
inspect next. Prefer stable identifiers and follow-up handles such as `id`,
`html_url`, `title`, `score`, `rank`, or provider timestamps. If a search row is
intentionally thin, expose enough stable identifiers for ordinary detail tables
...
Provider-ranked retrieval surfaces must be declared as `kind: search` functions.
Table filters are for exact lookup, required scoping, or ordinary provider-side
filtering on list/detail tables.

For `kind: search` functions, `search_limits` is required. `default_top_k` is
used when SQL does not specify a `LIMIT`, `max_top_k` caps candidates requested
per provider call, and `max_calls_per_query` caps provider calls for one SQL
query. Values must be positive. `default_top_k` and `max_top_k` are capped at
1000, `max_calls_per_query` is capped at 100, and
...
In DSL v3, put `universal_search` on the authored table function, as in the
`search_issues` example above:

- `id` is a source-stable route identifier, must match
  `[a-z][a-z0-9_]*`, and must be unique within the source.
- `execute: true` explicitly allows the route when the rest of the route
  validates. `execute: false` explicitly denies it.
- `query_arg` names the one exposed query argument, whose type must be exactly
  `Utf8`. An allowed query argument cannot have a default.
- Every other exposed argument on an allowed route must have a type-correct
  `default`.
- Optional `result` fields name authored result columns. `identity_fields` is
  ordered; `provider_id`, `title`, `url`, and `snippet` name scalar columns;
  `attributes` lists explicitly selected display fields.

DSL v4 table functions are generated from one imported surface, so author routes
...
universal_search:
  routes:
    issue_search:
      execute: true
      target:
        operation_id: search_issues_and_pull_requests
      query_input:
        location: query
        name: q
      result:
...
DSL v4 result fields are RFC 6901 JSON Pointers rooted at the imported result
row. Use escapes `~0` for `~` and `~1` for `/`. Manifest validation checks the
pointer syntax; during route resolution, every pointer must resolve to exactly
one imported result field. Identity and display fields must be scalar. An
attribute may select a structured value only when its schema permits bounded
canonical JSON.

To deny a DSL v4 route, keep the exact target and omit `query_input`:
...
- Any authored `universal_search` policy puts the source in explicit mode;
  unlisted routes are not inferred.
- Without authored policy, only one unambiguous, safe `kind: search` route can
  be inferred. Legacy `mode: search` filters never qualify.
- Duplicate or missing targets, ambiguous query inputs, invalid defaults, and
  unresolved result fields do not fall back to a guessed operation.
- `execute: true` is the source author's assertion that the target is
  retrieval-only and idempotent. Verify that assertion against the upstream
  API. Inference is stricter: HTTP operations must use a safe retrieval method,
  and MCP tools must declare read-only and idempotent annotations.
- Changing result identity or display mappings does not create a new
  authorization.

### MCP sources

MCP sources expose Model Context Protocol tools as SQL tables and source-scoped
table functions. The manifest remains the contract for SQL names, filters,
arguments, response shape, and columns; Coral does not introspect MCP tools at
query time.

Stdio MCP servers run as child processes:
...
If the Streamable HTTP server needs bearer authentication, declare the token as
a secret input and reference it from `server.auth`. OAuth credential metadata is
...
Streamable HTTP token refresh is not handled by the MCP transport itself. If a
server rejects an expired token, Coral surfaces an auth failure; refresh support
is owned by the shared OAuth credential flow.

Each tool call opens a fresh MCP session: stdio sources spawn a child process
per `tools/call`, and Streamable HTTP sources run the initialize handshake on
...
Query-time failures surface as one of the following structured error reasons.
Every reason carries `source` and `mcp_stage` metadata. Tool-scoped reasons
...
If the token was collected with an OAuth credential method, runtime auth still
uses the same stored secret input.
...
HTTP table filters can opt in to dependent predicate pushdown. This metadata is
table-local: it says a filter may be supplied from a runtime join key, not that
the table depends on a specific source pair.

The manifest contract is deliberately narrow:

| Field | Scope | Type | Default | Description |
| ----- | ----- | ---- | ------- | ----------- |
| `filters[].lookup_key` | filter | boolean | `false` | Allows the filter to be supplied from a runtime join key |

Lookup-key filters must use `mode: equality`. Do not mark ranked, capped, fuzzy,
or otherwise incomplete search-result filters as lookup keys; dependent joins rely
on complete filtered result sets to preserve SQL join correctness.

Dependent join execution buffers resolver rows and dependent fetch results
before producing joined output. Row, binding, and fanout limits are engine
runtime policy, not manifest metadata.
...
- `hint` (string): markdown guidance shown next to the fields that method
  collects — for example whether Coral registers the OAuth client dynamically,
  how to register a static OAuth app, what the callback URL is, or where to
  find an existing token. Keep each hint focused on the inputs that method needs
  rather than restating the other methods. Prefer a per-method `hint` over a
  single long input-level `hint` when the input offers several methods, since
  the fields below the hint change with the selected method.
...
`endpoints.device_authorization_url`, `endpoints.token_url`, and either static
client ID settings through `client.id.default` or `client.id.input`, or
`client.dynamic_registration`. Device-code OAuth must not declare a static
`client.secret`, and no callback URL, loopback listener, or PKCE setting is
required. A dynamic registration response may still return a client secret;
Coral uses it for token endpoint authentication according to
...
OAuth methods can also declare `client.dynamic_registration` with a
`registration_url`. When no static client ID is provided, Coral dynamically
registers a client during interactive credential setup and uses the returned
client ID, optional client secret, and token endpoint auth method for the
OAuth flow. Coral always sends `application_type: native` during Dynamic Client
Registration because Coral OAuth setup uses native-app flows: loopback
authorization-code redirects or device code. Source specs cannot configure a
`web` Dynamic Client Registration application type. Set
`request_refresh_token_grant: true` only when the provider supports registering
clients for refresh-token grants and the source should request refresh-token
support; otherwise Coral registers only the primary OAuth flow grant and still
stores refresh tokens opportunistically if the token endpoint returns them.

Use `resource` when the authorization server expects an OAuth resource
indicator, such as OAuth-protected Streamable HTTP MCP servers. Coral sends the
same resource value on authorization, device-code, token, and refresh requests.

OAuth URLs may include `{{input.KEY}}` templates for declared
...
domain. This applies to `endpoints.*`, `resource`, and
`client.dynamic_registration.registration_url`. OAuth URL templates do not
support secret inputs, filters, function arguments, state, or inline defaults.
...
The CLI also accepts the final loopback redirect URL in the terminal while it
waits, which lets users complete OAuth when their browser cannot reach the
machine running Coral directly.

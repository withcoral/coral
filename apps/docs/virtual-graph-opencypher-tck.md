...
603 scenarios, which is 37.21% of the full upstream scenario-definition inventory
and 46.45% of the read-candidate inventory.
...
The baseline currently contains 603 representative read-only scenarios:
...
- `With`: 40 scenarios for transparent scope filtering, scalar alias filters,
  ...
  multi-hop final `MATCH` chains after row-limited and aggregate `WITH`, and
  explicit rejection coverage for aggregate `WITH` chains that require staged
  planning.
...
- `Temporal`: 50 scenarios for DATE, LOCALDATETIME, and LOCALTIME map and
  ...
  access over native `date_part(...)` units, and adjacent temporal support
  boundaries, including `date(...)`, `localdatetime(...)`, `localtime(...)`,
  zoned `datetime(...)` / `time(...)`, `duration(...)`, unsupported temporal map
  forms, deferred component names, kind-mismatched components, arithmetic, and
  `duration.between(...)`.
...
The GraphQL baseline currently contains 147 representative read-only scenarios:

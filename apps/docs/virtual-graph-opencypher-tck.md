...
682 scenarios, which is 42.23% of the full upstream scenario-definition inventory
and 52.70% of the read-candidate inventory.
...
The baseline currently contains 682 representative read-only scenarios:
...
- `With`: 51 scenarios for transparent scope filtering, scalar alias filters,
  ...
  carry into staged optional finals, aggregate relationship-key carry into
  staged final `MATCH`, and explicit rejection coverage for aggregate `WITH`
  chains that require staged planning.

...
726 scenarios, which is 44.95% of the full upstream scenario-definition inventory
and 56.11% of the read-candidate inventory.
...
The baseline currently contains 726 representative read-only scenarios:
...
- `With`: 53 scenarios for transparent scope filtering, scalar alias filters,
  ...
  carry into staged optional finals, declaration-inferred endpoint labels for
  carried relationships, aggregate relationship-key carry into staged final
  `MATCH`, and explicit rejection coverage for aggregate `WITH` chains that
  require staged planning.
...
- `Temporal`: 103 scenarios for DATE, LOCALDATETIME, DATETIME, and LOCALTIME map
...
- `GraphMetadata`: 20 scenarios for `id(...)`, `elementId(...)`, `type(...)`,

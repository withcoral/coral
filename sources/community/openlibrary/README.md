...
Open Library APIs require no authentication, but users are expected to cache responses and avoid bulk harvesting. Default non-identified requests are strictly rate-limited. If you need bulk access, please use their [Data Dumps](https://openlibrary.org/developers/dumps). Read the full [Usage Guidelines](https://openlibrary.org/developers/api) for more details.

...
| `metadata` | Utf8 | Additional metadata as a JSON string |
...
Expected connectivity output (when test data exists):
...
Sample queries to verify each table:

```sql
-- Payments
SELECT payment_id, status, total_amount, currency,
       customer_id, customer_email, created_at
FROM dodopayments.payments
WHERE status = 'succeeded'
ORDER BY created_at DESC
LIMIT 5;
...
-- Refunds
SELECT refund_id, payment_id, status, amount, is_partial, created_at
FROM dodopayments.refunds
ORDER BY created_at DESC
LIMIT 5;

-- Disputes
SELECT dispute_id, payment_id, dispute_status, amount, created_at
FROM dodopayments.disputes
ORDER BY created_at DESC
LIMIT 5;

-- Payouts
SELECT payout_id, status, amount, currency, created_at
FROM dodopayments.payouts
ORDER BY created_at DESC
LIMIT 5;

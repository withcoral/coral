...
.PHONY: slack-spec slack-spec-fetch slack-spec-check
...
# ----------------------------------------------------------------------------
# Generated Slack OpenAPI descriptor
# ----------------------------------------------------------------------------
# Slack publishes no usable OpenAPI 3.0 description of its Web API, so
# sources/v4/slack/openapi.yaml is generated by tools/openapi-forge from Slack's
# reference docs, the response samples in slackapi/java-slack-sdk, and the
# request types in @slack/web-api.
#
# fetch and build are separate on purpose: builds read only the committed
# snapshot, so they are reproducible and an upstream change arrives as a
# reviewable snapshot diff rather than a surprise.
#
#   make slack-spec-fetch   # refresh the pinned snapshot from upstream
#   make slack-spec         # regenerate the descriptor and catalog preview
#   make slack-spec-check   # CI freshness check: non-zero exit if stale

slack-spec-fetch:
	npm ci --prefix tools/openapi-forge
	node tools/openapi-forge/src/index.ts fetch --api slack

slack-spec:
	npm ci --prefix tools/openapi-forge
	node tools/openapi-forge/src/index.ts build --api slack
	cargo run --locked -p xtask -- v4-preview sources/v4/slack/manifest.yaml \
	  --out sources/v4/slack/catalog-preview.md

slack-spec-check:
	npm ci --prefix tools/openapi-forge
	npm run check --prefix tools/openapi-forge
	node tools/openapi-forge/src/index.ts build --api slack --check
	cargo run --locked -p xtask -- v4-preview sources/v4/slack/manifest.yaml \
	  --out sources/v4/slack/catalog-preview.md --check

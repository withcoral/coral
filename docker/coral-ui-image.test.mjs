...
test('every Node image matches the Coral UI toolchain version', async () => {
  const [dockerfile, nodeVersionFile] = await Promise.all([
    read('./Dockerfile.coral-ui'),
    read('../apps/coral-ui/.node-version'),
  ])
  const nodeVersion = nodeVersionFile.trim()
  const nodeFromLines = dockerfile.match(/^FROM .*\bnode(?::\S+)?(?:\s|$).*$/gm) ?? []

  assert.ok(nodeVersion, 'apps/coral-ui/.node-version must not be empty')
  assert.ok(nodeFromLines.length > 0, 'Dockerfile.coral-ui must declare at least one Node image')
...
test('runtime stage is COPY-only, non-root, and health checked; build stages stay on the build platform', async () => {
  const source = await read('./Dockerfile.coral-ui')
...
test('image copies production output instead of published binaries', async () => {
  ...
  assert.match(source, /COPY --from=build \/src\/apps\/coral-ui\/build \.\/build/)

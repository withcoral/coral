...
mod tests {
    ...
    fn report_counts_current_baseline() {
        ...
        assert_eq!(report.scenario_count, 456);
        assert_eq!(report.expected_error_count, 47);
    ...
    fn report_counts_graphql_baseline() {
        ...
        assert_eq!(report.scenario_count, 65);
        assert_eq!(report.expected_error_count, 8);
        assert_eq!(report.feature_counts.get("Aggregation"), Some(&6));
        assert_eq!(report.feature_counts.get("RootSelection"), Some(&4));
        assert_eq!(report.feature_counts.get("ScalarFilters"), Some(&12));
        assert!(report.feature_floor_violations.is_empty());
        assert!(report.undeclared_features.is_empty());
        assert!(feature_floors_satisfied(&report));
    }
    ...
    fn upstream_report_classifies_feature_groups() {
        ...
        assert_eq!(report.coral_baseline_scenario_count, 456);
        assert_eq!(
            report
                .category_counts

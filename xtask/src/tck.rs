...
mod tests {
    ...
    fn report_counts_current_baseline() {
        ...
        assert_eq!(report.scenario_count, 396);
        assert_eq!(report.expected_error_count, 42);
        ...
        assert_eq!(report.feature_counts.get("ExistsSubquery"), Some(&2));
        assert_eq!(report.feature_counts.get("GraphMetadata"), Some(&19));
        assert_eq!(report.feature_counts.get("ListExpressions"), Some(&36));
        ...
        assert_eq!(report.feature_counts.get("Parameters"), Some(&3));
        assert_eq!(report.feature_counts.get("PatternComprehension"), Some(&2));
        assert_eq!(report.feature_counts.get("PathValues"), Some(&4));
        assert_eq!(report.feature_counts.get("Quantifiers"), Some(&12));
        assert_eq!(report.feature_counts.get("ReturnDistinct"), Some(&3));
        assert_eq!(report.feature_counts.get("ReturnProjection"), Some(&12));
        assert_eq!(report.feature_counts.get("ScalarExpressions"), Some(&60));
        assert_eq!(report.feature_counts.get("TypeConversion"), Some(&16));
        assert_eq!(report.feature_counts.get("Union"), Some(&17));
        assert_eq!(report.feature_counts.get("Unwind"), Some(&13));
        assert_eq!(report.feature_counts.get("VariableLengthPaths"), Some(&4));
        assert_eq!(report.feature_counts.get("Where"), Some(&24));
        assert_eq!(report.feature_counts.get("With"), Some(&24));
    ...
    fn report_counts_graphql_baseline() {
        ...
        assert_eq!(report.scenario_count, 46);
        assert_eq!(report.expected_error_count, 7);
        assert_eq!(report.feature_counts.get("Aggregation"), Some(&6));
        assert_eq!(report.feature_counts.get("RootSelection"), Some(&2));
        assert_eq!(report.feature_counts.get("ScalarFilters"), Some(&8));
        assert!(report.feature_floor_violations.is_empty());
        assert!(report.undeclared_features.is_empty());
        assert!(feature_floors_satisfied(&report));
    }
    ...
    fn upstream_report_classifies_feature_groups() {
        ...
        assert_eq!(report.coral_baseline_scenario_count, 396);
        assert_eq!(
            report
                .category_counts

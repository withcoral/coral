...
mod tests {
    ...
    fn report_counts_current_baseline() {
        ...
        assert_eq!(report.feature_counts.get("ListExpressions"), Some(&4));
        assert_eq!(report.feature_counts.get("LiteralExpressions"), Some(&2));
        assert_eq!(report.feature_counts.get("MathematicalFunctions"), Some(&4));
        assert_eq!(report.feature_counts.get("NullSemantics"), Some(&3));
        assert_eq!(report.feature_counts.get("OptionalMatch"), Some(&4));
        assert_eq!(report.feature_counts.get("PathValues"), Some(&3));
        assert_eq!(report.feature_counts.get("ScalarExpressions"), Some(&21));
        assert_eq!(report.feature_counts.get("TypeConversion"), Some(&3));
        assert_eq!(report.feature_counts.get("VariableLengthPaths"), Some(&4));
        assert_eq!(report.feature_counts.get("Where"), Some(&10));
        assert_eq!(report.feature_counts.get("With"), Some(&4));
    ...
    fn upstream_report_classifies_feature_groups() {
        ...
        assert_eq!(report.coral_baseline_scenario_count, 86);
        assert_eq!(
            report
                .category_counts

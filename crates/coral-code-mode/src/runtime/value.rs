...
pub(super) fn is_tagged_template_strings(
    scope: &mut v8::PinScope<'_, '_>,
    value: v8::Local<'_, v8::Value>,
) -> Result<bool, String> {
    if !value.is_array() {
...
pub(super) fn structured_result_to_json<'s>(
    ...
    let mut budget = StructuredResultBudget::new();
...
impl StructuredResultBudget {
    fn new() -> Self {
        Self {
            remaining_values: MAX_STRUCTURED_RESULT_CONTAINER_ENTRIES,
        }
    }
    ...
    fn consume_value(&mut self, path: &str) -> Result<(), String> {
        if self.remaining_values == 0 {
            return Err(format!(
                "code mode result exceeded the structured result entry limit of {MAX_STRUCTURED_RESULT_CONTAINER_ENTRIES} at {path}"
            ));
        }
    ...
    fn ensure_container_entries(&self, path: &str, len: usize) -> Result<(), String> {
        ...
        if len > self.remaining_values {
            return Err(format!(
                "code mode result exceeded the structured result entry limit of {MAX_STRUCTURED_RESULT_CONTAINER_ENTRIES} before serializing {path}"
            ));
        }
        Ok(())
    }

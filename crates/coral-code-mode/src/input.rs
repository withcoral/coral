...
fn normalize_function_tool_input(tool_name: &str, input: Option<Value>) -> Result<Value, String> {
    match (tool_name, input) {
        ...
        ("sql", Some(Value::Object(mut input)))
...
fn normalize_freeform_tool_input(tool_name: &str, input: Option<Value>) -> Result<Value, String> {
    match input {
        ...
        Some(other) => Err(format!(
            "tools.{tool_name} expects a string argument, got {}",
            json_type_name(&other)
        )),
    }
}
...
fn tagged_template_sql_input(input: &mut Map<String, Value>) -> Result<Value, String> {
    let template = input
        .remove(CODE_MODE_TAGGED_TEMPLATE_KEY)
...
mod tests {
    ...
    #[test]
    fn normalize_sql_tagged_template_input() {
    ...
    fn normalize_freeform_rejects_non_string_input() {
        ...
        assert_eq!(
            error,
            "tools.write_file expects a string argument, got object"
        );
    }
}

            ...
            maybe_event = async {
                ...
                match event {
                    ...
                    RuntimeEvent::ToolCall {
                        ...
                        if let Err(error_text) = increment_nested_call_count(&inner, &cell_id).await
                        {
...
async fn increment_nested_call_count(inner: &Inner, cell_id: &str) -> Result<(), String> {
...
async fn merge_stored_value_updates(
    inner: &Inner,
    _runtime_stored_values: HashMap<String, JsonValue>,
...
"#
        ...
        assert!(
            error_text.ends_with("[truncated]"),
            "expected truncated error, got {error_text:?}"
        );
        assert!(
            error_text.len() < 100,
...
"#
    ...
    #[tokio::test]
    async fn max_output_tokens_rejects_oversized_result() {
        let service = CodeModeService::new();

        let response = service
            .execute(ExecuteRequest {
                source: r#"globalThis.__coral_code_mode_result = "x".repeat(100);"#.to_string(),
                yield_time_ms: None,
    ...
    #[tokio::test]
    async fn max_output_tokens_rejects_sparse_arrays_before_materializing() {
        let service = CodeModeService::new();

        let response = service
            .execute(ExecuteRequest {
                source: r#"return new Array(1000);"#.to_string(),
                yield_time_ms: None,
        ...
        assert!(
            error_text
                .expect("budget error")
                .contains("exceeded max_output_tokens before serializing result")
        );
    }
...
"#
        ...
        assert_eq!(
            ...
            WaitOutcome::LiveCell(RuntimeResponse::Result {
                cell_id: "1".to_string(),
                content_items: Vec::new(),
                stored_values: expected_store.clone(),
                result: None,
                error_text: None,
            })
        );
...
"#
        ...
        assert_eq!(second.input, Some(json!("select 2")));
...
"#
        ...
        assert_eq!(result.get("i"), Some(&JsonValue::from(100)));
        assert!(
            result
                .get("error")
...
"#
        ...
        assert_eq!(
            result.get("i"),
            Some(&JsonValue::from(MAX_NESTED_CALLS_PER_CELL))
        );
        assert!(
            result
                .get("error")
...
globalThis.__coral_code_mode_result = {
  ...
  process: Object.hasOwn(globalThis, "process"),
  require: Object.hasOwn(globalThis, "require"),
  Deno: Object.hasOwn(globalThis, "Deno"),

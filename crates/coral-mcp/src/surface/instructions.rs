...
static INITIAL_INSTRUCTIONS_WITH_SQL: &str = "You are connected to Coral, a provider capability system. Discover available capabilities with `search`, inspect compact invocation/projection metadata with `describe`, and run JavaScript Code Mode with `exec` and `wait`. If Coral blocks or confuses you, call the MCP `feedback` tool directly instead of searching for it as a capability. Generated provider methods are global under `tools.*`; use `full_path` from `search` or `describe` to call them from Code Mode. Coral helpers are global under `coral.*`. Generated provider methods reject on provider or transport failure by default; the rejection is catchable with try/catch, and uncaught rejections fail the run. Successful calls return `{ ok, complete, partial, errors, source_status, value, error, envelope }`; after checking `response.ok`, read provider data from `response.value`. Use `{ allowErrorResult: true }` only when deliberately collecting raw `{ ok: false }` provider diagnostics. Use typed refs such as `typescript:*`, `sql_table:*`, and `sql_function:*`; when a SQL projection is available, call it from Code Mode through `coral.sql.query`. Do not guess ambiguous untyped names: retry `describe` with a typed ref or full_path when candidates are returned. Use `describe` with `view: \"detailed\"` only when full provider schemas are needed.";
static INITIAL_INSTRUCTIONS_TYPESCRIPT_ONLY: &str = "You are connected to Coral, a provider capability system. Discover available capabilities with `search`, inspect compact invocation metadata with `describe`, and run JavaScript Code Mode with `exec` and `wait`. If Coral blocks or confuses you, call the MCP `feedback` tool directly instead of searching for it as a capability. Generated provider methods are global under `tools.*`; use `full_path` from `search` or `describe` to call them from Code Mode. Coral helpers are global under `coral.*`. Generated provider methods reject on provider or transport failure by default; the rejection is catchable with try/catch, and uncaught rejections fail the run. Successful calls return `{ ok, complete, partial, errors, source_status, value, error, envelope }`; after checking `response.ok`, read provider data from `response.value`. Use `{ allowErrorResult: true }` only when deliberately collecting raw `{ ok: false }` provider diagnostics. Use typed refs such as `typescript:*`. SQL projection refs and `coral.sql.query` are hidden by runtime exposure. Do not guess ambiguous untyped names: retry `describe` with a typed ref or full_path when candidates are returned. Use `describe` with `view: \"detailed\"` only when full provider schemas are needed.";

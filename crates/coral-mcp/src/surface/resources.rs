...
static INITIAL_INSTRUCTIONS_SUFFIX: &str = "Use `describe` for table or table-function metadata and `list_columns` for table columns, use `sql` against `coral.tables`, `coral.columns`, `coral.filters`, `coral.table_functions`, and `coral.inputs` for deeper discovery, then answer with set-based SQL through `sql`. Prefer one SQL statement with joins, CROSS JOIN, CTEs, subqueries, and aggregates over row-by-row tool calls. You MUST use `add_function` when it would improve future discovery or compress a useful query workflow into fewer or simpler future calls.";

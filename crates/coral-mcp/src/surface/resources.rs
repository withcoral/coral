...
pub(crate) fn guide_resource_content(
    ...
    sources_section.push_str(
        "- coral: System catalog schema. Query `coral.tables`, `coral.columns`, `coral.filters`, `coral.table_functions`, `coral.inputs`, and `coral.task_queries` like database catalog tables to discover queryable tables, table functions, columns, filter metadata, and retained task SQL activity.\n",
    );

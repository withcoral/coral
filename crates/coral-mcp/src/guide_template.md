...
- `list_catalog` shows queryable source tables and source-scoped table functions in pages; pass `schema`, `kind`, `limit`, and `offset` to narrow large catalogs. Omit `kind` or pass `null` to list all item kinds. Pass `schema: "coral"` to inspect Coral's system metadata tables.
- `search_catalog` searches table and table-function names, descriptions, arguments, result columns, guides, and required filters with a Rust regex, including `coral.*` system tables when the pattern matches them; use it before broad SQL metadata scans when you know part of the catalog item you need.

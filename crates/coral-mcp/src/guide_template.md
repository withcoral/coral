...
- `search_catalog` searches table and table-function names, descriptions, ordinary table columns, arguments, result columns, guides, and required filters with a Rust regex; use it before broad SQL metadata scans when you know part of the catalog item you need. Ordinary table hits include a capped `column_preview` with `column_count` and `omitted_column_count`; use `list_columns` for full columns.

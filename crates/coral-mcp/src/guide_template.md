...
- `coral://tables` shows table summaries for query-visible source tables and Coral system tables, including the metadata catalog and workspace-scoped `coral.tasks`, `coral.task_queries`, and `coral.task_query_relations` history.
- Task history is best-effort activity, not an audit log. It has no implicit order; use `ORDER BY started_at, query_id` for stable query display, and expect relation rows only for successful queries.

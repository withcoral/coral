...
- `describe` accepts `schema` and a bare `surface` name, plus `catalog` for a three-part table. Coral resolves whether the surface is a table or table function. Tables return compact guide, filter, and column-count metadata. Table functions return their arguments, result columns, guide, and a minimal SQL call example. Use `coral.columns` when you need full table column details.

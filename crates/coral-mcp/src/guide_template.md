...
- The initial preview is byte-budgeted. If `sql` returns a metadata-only preview with `limit: 0`, `rows: []`, and `next_offset: 0`, call `result_get` with a column projection or rerun with filters, `LIMIT`, or smaller expressions.

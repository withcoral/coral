...
- `describe` accepts `schema` and a bare `surface` name, plus `catalog` for a three-part table. Coral resolves an exact table or table function and returns `reason: missing` when no exact target exists. Tables return compact guide, filter, and column-count metadata. Table functions return their arguments, result columns, and guide. Use `coral.columns` when you need full table column details.

...
- If two large `result_get` pages do not answer the question, stop paging and rerun the SQL with stronger filters, aggregates, ordering, `LIMIT`, or a narrower projection. Treat deep-paging guidance in `result_get` responses as a prompt to refine the query instead of following `has_more` mechanically.

...
- When `sql_codemode` is available, use it for multi-step analysis that needs session-local `CREATE TEMP TABLE ... AS SELECT` statements. It returns one `results` entry per query statement; temporary-table creation statements do not produce rows.

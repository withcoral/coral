...
- `coral.task_queries` shows retained SQL activity for tasks in the current workspace. Filter by `task_id` and order by `executed_at, query_id` for deterministic execution-start order.
- `coral://tables` shows table summaries for query-visible source tables and Coral catalog tables, including `coral.tables`, `coral.columns`, `coral.filters`, `coral.table_functions`, `coral.inputs`, and `coral.task_queries`; those catalog tables provide richer SQL metadata.

...
- `describe` accepts exactly one `table` or `table_function` target object. Tables return compact guide, filter, and column-count metadata. Table functions return their arguments, result columns, guide, and a minimal SQL call example. Use `coral.columns` when you need full table column details.

...
- If two large `result_get` pages do not answer the question, stop paging. Use the rows already seen and `row_count`; only rerun SQL when you can name a specific filter, aggregate, ordering, `LIMIT`, or narrower projection that directly answers the question. Treat deep-paging guidance in `result_get` responses as a stop signal, not a reason to follow `has_more` mechanically.
- The initial preview is byte-budgeted. If `sql` returns a metadata-only preview with `limit: 0` and `rows: []`, use the provided `next_call` for handled results, or rerun with filters, `LIMIT`, or smaller expressions when the response is `preview_only` and has no stored handle.

...
use crate::{
    McpOptions,
    result_handles::{ResultHandles, query_result_status},
    surface::{
...
impl CoralMcpServer {
    pub(crate) fn new(app: &AppClient, options: McpOptions) -> Self {
        Self {
            ...
            result_handles: ResultHandles::new(),
            options,
        }
    }
    ...
    async fn execute_sql_value(&self, sql: &str) -> Result<Value, tonic::Status> {
        ...
        self.result_handles.sql_value(result)
    }
    ...
    fn result_get_tool_result(
        ...
        let value = self.result_handles.page_value(
            arguments.result_id,
            arguments.offset,
            arguments.limit,
            arguments.columns.as_deref(),
        )?;

...
use coral_spec::backends::file::{PartitionColumnSpec, PartitionPathSpec};
...
impl PartitionDataType {
    fn try_from_manifest(name: &str, data_type: ManifestDataType) -> Result<Self> {
        match data_type {
            ...
            ManifestDataType::Timestamp => Err(unsupported_timestamp_partition(name)),
        }
    }
...
impl PartitionColumn {
    fn try_from_spec(spec: &PartitionColumnSpec) -> Result<Self> {
        let name = spec.name.clone();
        let manifest_data_type = spec
            .manifest_data_type()
            .map_err(|error| DataFusionError::Execution(error.to_string()))?;
        let data_type = PartitionDataType::try_from_manifest(&name, manifest_data_type)?;
        Ok(Self {
            name,
            data_type,
...
impl PartitionColumns {
    pub(super) fn try_new(specs: &[PartitionColumnSpec]) -> Result<Self> {
        let columns = specs
            .iter()
            .map(PartitionColumn::try_from_spec)
...
fn unsupported_timestamp_partition(partition_name: &str) -> DataFusionError {
    DataFusionError::Plan(format!(
        "partition '{partition_name}' uses type=Timestamp, which is not supported for backend=file path partitions"
    ))
}

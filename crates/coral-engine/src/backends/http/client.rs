...
mod tests {
    ...
    #[tokio::test]
    async fn execute_inflight_does_not_share_different_limits() {
        let client = test_client();
        let calls = Arc::new(AtomicUsize::new(0));
        let current = Arc::new(AtomicUsize::new(0));
        let max_concurrent = Arc::new(AtomicUsize::new(0));
        let barrier = Arc::new(Barrier::new(2));
        let limit_one_key = request_key();
        let mut limit_two_key = request_key();
        limit_two_key.sql_limit = Some(2);

        let first = client.execute_inflight(limit_one_key, {
            let calls = Arc::clone(&calls);
            ...
            move || {
                ...
                async move {
                    ...
                    Ok(Some((json!({"limit": 1}), None)))
                }
            }
        });
        let second = client.execute_inflight(limit_two_key, {
            let calls = Arc::clone(&calls);
            let current = Arc::clone(&current);
            let max_concurrent = Arc::clone(&max_concurrent);
            let barrier = Arc::clone(&barrier);
        ...
        .await
        .expect("different-limit requests should not hang");

        assert_eq!(calls.load(Ordering::SeqCst), 2);
        assert_eq!(max_concurrent.load(Ordering::SeqCst), 2);
        assert_eq!(
            first.expect("limit 1 succeeds"),
            Some((json!({"limit": 1}), None))
        );
        assert_eq!(
            second.expect("limit 2 succeeds"),
            Some((json!({"limit": 2}), None))
        );
    }

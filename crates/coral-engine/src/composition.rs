...
/// How much SQL-local filtering has happened before this source observation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SourceObservationScope {
    /// Rows have been mapped into the declared table shape before SQL-local
    /// projection. Some backend/request-side pushdown may already have happened.
    MappedRowsBeforeProjection,
    /// Rows have been mapped after a source adapter evaluated local exact
    /// filters, but before SQL-local projection.
    MappedRowsAfterLocalFilterBeforeProjection,
}
...
pub struct SourceScanObservation<'a> {
    ...
    /// Scope of the observed mapped rows.
    pub observation_scope: SourceObservationScope,

...
//! `reference_validation` (context-free reference checks), `staged` (WITH
//! staged-query planning), `variable_rename` (IR rename visitors),
//! `scalar_builders` (pure IR construction), `subquery` (scoped subquery lowering).
...
fn compile_projected_graph_object_property_order_expression(
    expression: &Expression,
    projections: &[Projection],
    plan: &GraphPlan,

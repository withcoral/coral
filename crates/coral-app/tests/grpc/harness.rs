...
impl GrpcHarness {
    pub(crate) async fn new() -> Self {
        ...
        Self::start_with_parts(temp_dir, config_dir, FeatureOverrides::default(), true).await
    }

    pub(crate) async fn new_empty() -> Self {
        let temp_dir = TempDir::new().expect("temp dir");
        let config_dir = temp_dir.path().join("coral-config");
        Self::start_with_parts(temp_dir, config_dir, FeatureOverrides::default(), false).await
    }
    ...
    pub(crate) async fn new_with_observed_values_search() -> Self {
        ...
        Self::start_with_parts(temp_dir, config_dir, feature_overrides, true).await
    }
    ...
    async fn start_with_builder(
        ...
        if create_default_workspace {
            let result = app
                .workspace_client()
                .create_workspace(Request::new(coral_api::v1::CreateWorkspaceRequest {
                    workspace: Some(default_workspace()),
                }))
                .await;
            assert!(
                result.is_ok()
                    || result.is_err_and(|status| status.code() == tonic::Code::AlreadyExists),
                "create default test workspace"
            );
        }
        Self {
            temp_dir,

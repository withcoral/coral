...
impl TryFrom<TraceSummaryRow> for TraceSummaryRecord {
    ...
    fn try_from(value: TraceSummaryRow) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            ...
            operation_kind: operation_kind_from_db(&value.operation_kind)?,
            operation_name: value.operation_name,
            invocation_kind: invocation_kind_from_db(&value.invocation_kind)?,
        })
    }
}
    ...
    pub(crate) async fn upsert(&mut self, summary: &TraceSummaryRecord) -> Result<(), DbError> {
        ...
        let statement = Query::insert()
            ...
            .values_panic([
                ...
                Expr::val(span_count),
                Expr::val(row_count),
                Expr::val(operation_kind_to_db(summary.operation_kind)),
                Expr::val(summary.operation_name.clone()),
                Expr::val(invocation_kind_to_db(summary.invocation_kind)),
            ])
            .on_conflict(
...
fn operation_kind_to_db(kind: StoredTraceOperationKind) -> String {
    match kind {
        StoredTraceOperationKind::Unspecified => "unspecified",
        StoredTraceOperationKind::Query => "query",
        StoredTraceOperationKind::Search => "search",
        StoredTraceOperationKind::Tool => "tool",
        StoredTraceOperationKind::Other => "other",
    }
    .to_string()
}

fn operation_kind_from_db(kind: &str) -> Result<StoredTraceOperationKind, DbError> {
    match kind {
        "unspecified" => Ok(StoredTraceOperationKind::Unspecified),
        "query" => Ok(StoredTraceOperationKind::Query),
        "search" => Ok(StoredTraceOperationKind::Search),
        "tool" => Ok(StoredTraceOperationKind::Tool),
        "other" => Ok(StoredTraceOperationKind::Other),
        other => Err(DbError::CorruptData(format!(
            "database contains invalid trace operation kind '{other}'"
        ))),
    }
}

fn invocation_kind_to_db(kind: StoredTraceInvocationKind) -> String {
    match kind {
        StoredTraceInvocationKind::Unspecified => "unspecified",
        StoredTraceInvocationKind::Direct => "direct",
        StoredTraceInvocationKind::Mcp => "mcp",
    }
    .to_string()
}

fn invocation_kind_from_db(kind: &str) -> Result<StoredTraceInvocationKind, DbError> {
    match kind {
        "unspecified" => Ok(StoredTraceInvocationKind::Unspecified),
        "direct" => Ok(StoredTraceInvocationKind::Direct),
        "mcp" => Ok(StoredTraceInvocationKind::Mcp),
        other => Err(DbError::CorruptData(format!(
            "database contains invalid trace invocation kind '{other}'"
        ))),
    }
}
...
mod tests {
    ...
    fn summary(
        ...
        TraceSummaryRecord {
            ...
            row_count: 0,
            row_count_recorded,
            operation_kind: StoredTraceOperationKind::Query,
            operation_name: "sql".to_string(),

...
mod tests {
    ...
    #[tokio::test]
    async fn source_manifest_repository_round_trips_across_configured_backends() {
    ...
    #[tokio::test]
    async fn source_manifest_repository_rejects_manifest_without_source_across_configured_backends()
    {
        let temp = tempdir().expect("temp dir");
        let layout = AppStateLayout::discover(Some(temp.path().join("coral"))).expect("layout");
        let db = open_sqlite(&layout).await;

        assert_source_manifest_repository_rejects_manifest_without_source(&db).await;
        if let Some(db) = open_configured_postgres().await {
            assert_source_manifest_repository_rejects_manifest_without_source(&db).await;
        }
    }
    ...
    async fn assert_source_manifest_repository_rejects_manifest_without_source(db: &CoralDb) {

...
pub(super) fn validate_id_token(
    ...
    let mut matching = jwks.keys.iter().filter(|key| {
        key.common.key_id.as_deref() == Some(kid) && key.common.key_algorithm == Some(key_algorithm)
    });
    let key = matching.next().ok_or(())?;
    if matching.next().is_some() || !valid_key(key, algorithm) {
        return Err(());
    }
...
fn validate_claims(
    ...
    if subject.is_empty() || subject.len() > MAX_SUBJECT_BYTES || !subject.is_ascii() {
        return Err(());
    }
    ...
    let principal = claims
        ...
        .filter(|principal| !principal.is_empty() && principal.trim() == *principal)
        .ok_or(())?
        .to_string();
...
pub(in crate::auth) mod tests {
    ...
    fn provider(extra: &str) -> OidcProviderConfig {
    ...
    fn validates_mandatory_oidc_claims_and_time_types() {
        ...
        for claims in invalid {
            assert!(validate_claims(&provider, "expected-nonce", &claims).is_err());
        }
        ...
        assert!(validate_claims(&provider, "expected-nonce", &multiple).is_ok());
    }
    ...
    fn validates_identity_projection_and_required_claim_semantics() {
        ...
        for (field, value) in [
            ...
            assert!(validate_claims(&provider, "expected-nonce", &rejected).is_err());
        }
    }
}

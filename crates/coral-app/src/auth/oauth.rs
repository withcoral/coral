...
impl OidcAuthConfig {
    ...
    pub fn load(config_dir_override: Option<PathBuf>) -> Result<Option<Self>, String> {
        ...
        let session = SessionTokenIssuer::load(&layout)?
...
impl AuthConfigFile {
    fn build(self, session: SessionTokenIssuer) -> Result<Option<OidcAuthConfig>, String> {
...
mod tests {
    ...
    use super::{OidcAuthConfig, RunningOidcAuthServer};
    ...
    fn validates_configuration_fail_closed_without_leaking_secrets() {
        ...
        assert!(reject(&client_registry).contains("unknown field `clients`"));

    ...
    ResBody: Default + Send + 'static,
{
    ...
    fn call(&mut self, mut request: http::Request<B>) -> Self::Future {
        let method = GrpcServerMethod::from_path(request.uri().path());
        if let Some(method) = method.clone() {
            request.extensions_mut().insert(method);
        }
        let method_metadata = method.map_or_else(
...
pub enum OAuthProgressProto {
    /// Authorization details for clients to display while OAuth is pending.
    Authorization(coral_api::v1::OAuthCredentialAuthorization),
    /// Completion metadata emitted after OAuth succeeds.

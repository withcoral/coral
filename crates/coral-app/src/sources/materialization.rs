...
fn materialize_openapi_surface(
    ...
    let expected_sha256 = surface.descriptor.sha256().ok_or_else(|| {
        AppError::FailedPrecondition(format!(
            "OpenAPI surface '{}' must declare a descriptor sha256",
            surface.id
        ))
    })?;
    if observed_sha256 != expected_sha256 {
        return Err(AppError::FailedPrecondition(format!(
            "OpenAPI descriptor for source '{}' surface '{}' has sha256 {observed_sha256}; expected {}",
            manifest.common.name, surface.id, expected_sha256
        )));
    }

...
"#
    ...
    #[test]
    fn observed_cleanup_bumps_source_generation_when_sqlite_file_is_absent() {
        let temp = TempDir::new().expect("temp dir");
        let layout =
            AppStateLayout::discover(Some(temp.path().join("coral-config"))).expect("layout");
        layout.ensure().expect("ensure layout");
        ...
        assert!(layout.search_sqlite_file(&workspace_name).exists());
        let generation = SqliteObservedValuesStore::new(layout)
            .current_generations(&workspace_name, source_name.as_str())
            .expect("observed-values generation");
        assert_eq!(generation.source_generation, 1);

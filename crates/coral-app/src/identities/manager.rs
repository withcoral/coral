...
impl UserOwnedIdentityManager {
    pub(crate) fn new(layout: AppStateLayout, identity_specs: IdentitySpecManager) -> Self {
        Self::new_with_store(
            identity_specs,
            Arc::new(FileUserOwnedIdentityStore::new(layout)),
        )
    }
...
impl UserOwnedIdentityStore for FileUserOwnedIdentityStore {
    ...
    async fn delete_identity(
        ...
        tokio::task::spawn_blocking(move || {
            let owner_key = validate_identity_owner_key(&owner_key)?;
            let _material_lock =
                Self::material_lock_for_layout(&store.layout, &owner_key, identity_name.as_str())?;
            let _state_lock = FileLock::exclusive(store.layout.state_lock())?;
            let manifest_path = store
                .layout
...
impl UserOwnedIdentityMaterialGuard for FileUserOwnedIdentityMaterialGuard {
    async fn read_material(&self) -> Result<BTreeMap<String, String>, AppError> {
        let path = self
            .layout
            .user_owned_identity_material_file(&self.owner_key, &self.identity_name);
    ...
    async fn write_material(&self, material: &BTreeMap<String, String>) -> Result<(), AppError> {
        ...
        let material = material.clone();
        tokio::task::spawn_blocking(move || {
            let path = layout.user_owned_identity_material_file(&owner_key, &identity_name);
...
impl SourceIdentityProvider for UserOwnedIdentityManager {
    ...
    async fn resolve_source_identity(
        ...
        self.resolve_identity(&owner, identity_name.as_str()).await
    }
}
...
mod tests {
    ...
    fn github_local_name() -> UserOwnedIdentityName {
        UserOwnedIdentityName::new("github_local").expect("identity name")
    }
...
"
    ...
    #[test]
    fn identity_oauth_persistence_tracks_only_provided_legacy_inputs() {
        let manifest = demo_oauth_identity_spec("input: DEMO_OAUTH_CLIENT_ID");
        let oauth = oauth_method(&manifest.name, &manifest.config).expect("oauth method");
        let provided = material("DEMO_OAUTH_CLIENT_ID", "legacy-client");

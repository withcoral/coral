...
impl CatalogDocumentClass {
    /// Fixed SQL fragment over the `d` alias of `catalog_documents` — never
    /// built from caller input. Both backends share the vocabulary.
    pub(crate) fn doc_kind_predicate(self) -> &'static str {
        match self {

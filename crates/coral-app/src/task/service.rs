...
fn task_status_from_proto(status: i32) -> Result<DomainTaskStatus, crate::bootstrap::AppError> {
    match ProtoTaskStatus::try_from(status) {
        Ok(ProtoTaskStatus::Completed) => Ok(DomainTaskStatus::Completed),
        Ok(ProtoTaskStatus::Failed) => Ok(DomainTaskStatus::Failed),
        Ok(ProtoTaskStatus::Unspecified) => Err(crate::bootstrap::AppError::InvalidInput(
            "task status must be completed or failed".to_string(),
        )),
...
fn task_status_to_proto(status: DomainTaskStatus) -> ProtoTaskStatus {
    match status {
        DomainTaskStatus::Completed => ProtoTaskStatus::Completed,
        DomainTaskStatus::Failed => ProtoTaskStatus::Failed,
    }
}
...
mod tests {
    ...
    fn service() -> (TempDir, AppStateLayout, TaskService) {
        ...
        let task = TaskManager::new(TaskStore::new(layout.clone()));
    ...
    #[tokio::test]
    async fn end_task_persists_completed_status() {
        let (_dir, layout, service) = service();
        ...
        let response = service
            .end_task(Request::new(EndTaskRequest {
                ...
                task_status: TaskStatus::Completed as i32,
            }))
            .await
            .expect("end task")
            .into_inner();
        ...
        assert_eq!(task_end.task_status, TaskStatus::Completed as i32);

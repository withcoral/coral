//! CLI entrypoint logic for the local Coral app.
...
/// Runs the CLI using the default local client bootstrap.
///
/// # Errors
///
/// Returns an error if local client bootstrap, transport operations, or output
/// rendering fails.
pub async fn run() -> Result<(), anyhow::Error> {
    let cli = Cli::parse();
    let bootstrap = bootstrap::bootstrap().await?;
    run_with_app(cli, bootstrap.app).await
}

/// Runs the CLI using a caller-provided client bootstrap strategy.
///
/// # Errors
///
/// Returns an error if client bootstrap, transport operations, or output
/// rendering fails.
pub async fn run_with_client_builder(client_builder: ClientBuilder) -> Result<(), anyhow::Error> {
    let cli = Cli::parse();
    let app = client_builder.build().await?;
    run_with_app(cli, app).await
}

async fn run_with_app(cli: Cli, app: AppClient) -> Result<(), anyhow::Error> {
    match cli.command {
        Command::Sql(args) => {

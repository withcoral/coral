...
async fn run_source_add(
    ...
    let Some(response) = (match (name, file) {
        (Some(name), None) => run_source_add_bundled(app, workspace, &name, interactive).await?,
        (None, Some(file)) => run_source_add_file(app, workspace, file, interactive).await?,
...
async fn run_source_add_bundled(
    ...
    if interactive {
        let collected = source_ops::collect_inputs_confirming_hosts(
            ...
            |variables| {
                source_ops::resolve_bundled_source_hosts(app, workspace, &available.name, variables)
            },
        )
        .await?;
        ...
        // Collect every input from the environment once. This reads each
        // variable exactly once and surfaces all missing required inputs
        // (variables and secrets) together, before the host list is shown.
        ...
        let hosts = source_ops::resolve_bundled_source_hosts(
            app,
            workspace,
            &available.name,
            variables.clone(),
...
async fn run_source_add_file(
    ...
    if interactive {
        ...
        Ok(Some(
            source_ops::import_source_with_credentials(app, workspace, manifest_yaml, collected)
                .await?,
        ))
    } else {

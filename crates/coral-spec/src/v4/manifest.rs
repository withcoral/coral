...
impl SurfaceDescriptor {
    ...
    pub fn sha256(&self) -> Option<&str> {
        match self {
            Self::Url { sha256, .. } | Self::File { sha256, .. } => Some(sha256),
            Self::McpServer { .. } => None,
        }
    }
}
...
fn parse_mcp_surface(
    ...
    if raw_surface.sha256.is_some() {
        return Err(ManifestError::validation(format!(
            "source '{source_name}' MCP surface '{}' must not declare sha256",
            raw_surface.id
        )));
    }
...
fn parse_openapi_descriptor(
    ...
    let sha256 = surface.sha256.as_deref().ok_or_else(|| {
        ManifestError::validation(format!(
            "source '{source_name}' surface '{}' must declare sha256",
            surface.id
        ))
    })?;

pub(super) fn fallback_operation_id(method: &str, path: &str) -> String {
...
pub(super) fn entity_name_from_ref(reference: &str) -> String {
    reference
        .rsplit('/')
        .next()
...
pub(super) fn type_id_from_ref(reference: &str) -> String {
    normalize_identifier(reference.rsplit('/').next().unwrap_or(reference), "type")
}

pub(super) fn entity_name_from_path(path: &str) -> String {
    path.split('/')
...
pub(super) fn pluralize(value: &str) -> String {
    if value.ends_with('s') {
        value.to_string()

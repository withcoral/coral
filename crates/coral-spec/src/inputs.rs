...
fn render_oauth_url_template(
    ...
    let context = format!("OAuth {label}");
...
fn validate_oauth_url_template_prefix(context: &str, raw_prefix: &str) -> Result<()> {
...
fn validate_oauth_url_template(
    ...
    if !has_required_variable {
        let url = Url::parse(&rendered)
            .map_err(|error| ManifestError::validation(format!("{context} is invalid: {error}")))?;
        crate::validate_https_or_loopback_scheme(&context, &url)
            .map_err(ManifestError::validation)?;
        validate_oauth_url_fragment_policy(&context, &url, fragment_policy)?;
        ...
        // any known prefix now; `render_oauth_url_template` validates the fully
        // rendered URL before use.
        validate_oauth_url_template_prefix(&context, prefix)?;
    }
...
",
    ...
    fn endpoint_urls_reject_plaintext_rendered_source_input_templates() {
        ...
        let error = oauth
            .endpoint_urls(&source_inputs)
            .expect_err("plaintext rendered endpoint should fail");

        assert!(
            error.to_string().contains("OAuth token URL must use https"),
            "unexpected error: {error}"
        );
    }
    ...
    fn resource_rendering_rejects_rendered_fragments_from_source_inputs() {
        let mut oauth = oauth_endpoint_spec(
            Some("https://provider.example.com/oauth/authorize"),
            None,
            "https://provider.example.com/oauth/token",
        );

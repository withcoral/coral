...
fn validate_identity_oauth_method_fields(
    ...
    validate_known_fields(name, "oauth.method.client", client, &["id", "secret"])?;
    validate_nested_known_fields(
        name,
        "oauth.method.client",
        client,
...
oauth:
  ...
  method:
    ...
    client: {id: {default: demo-client}, secret: {input: OAUTH_CLIENT_SECRET, transport: request_body}}",
            ...
            "missing client secret declaration should fail",
  ...
  method:
    ...
    client: {id: {input: OAUTH_CLIENT_ID}}",
            ...
            "missing client id declaration should fail",
            "must reference a declared variable input",
        );
    }

    #[test]
    fn rejects_unknown_oauth_method_field() {
        expect_validation_error(
            &demo_oauth(
                r"oauth:
  method:
    flow: {type: authorization_code, pkce: required}
    redirect_uri: http://127.0.0.1:53682/oauth/callback
    redirect_uri_port_mdoe: fixed
    endpoints: {authorization_url: 'https://provider.example.com/oauth/authorize', token_url: 'https://provider.example.com/oauth/token'}
    client: {id: {default: demo-client}}",
            ),
            "unknown oauth method field should fail",
            "unsupported field 'redirect_uri_port_mdoe'",
        );
    }

    #[test]
    fn rejects_unknown_nested_oauth_method_field() {
        expect_validation_error(
            &demo_oauth(
                r"oauth:
  method:
    ...
    client:
      ...
      secrett: {input: OAUTH_CLIENT_SECRET, transport: request_body}",
            ),
            "unknown nested oauth method field should fail",
            "oauth.method.client has unsupported field 'secrett'",
        );
    }

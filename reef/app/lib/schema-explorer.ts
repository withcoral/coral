...
import { catalogClient, WORKSPACE } from './coral-clients'
...
export function requiredFilterSet(table: TableDef): Set<string> {
  return new Set(table.requiredFilters)
}
...
async function listColumnsForTable(schemaName: string, tableName: string): Promise<ColumnDef[]> {
  ...
  if (pagination.totalCount > nextOffset) {
    const remainingPages = await Promise.all(
      pageOffsets(nextOffset, pagination.totalCount, COLUMN_PAGE_LIMIT).map((offset) =>
        listColumnsPage(schemaName, tableName, offset),
      ),
    )

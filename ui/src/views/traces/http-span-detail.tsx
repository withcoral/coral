...
function bodySpanAttributes(bodySpans: TraceSpan[], parentSpanId: string, kind: BodyKind): Record<string, unknown> | undefined {
...
export function HttpSpanDetail({
  ...
  const requestBodyTruncated = attrBool(requestBodyAttrs?.[REQUEST_BODY_TRUNCATED_ATTR] ?? attrs[REQUEST_BODY_TRUNCATED_ATTR])
  const responseBodyTruncated = attrBool(responseBodyAttrs?.[RESPONSE_BODY_TRUNCATED_ATTR] ?? attrs[RESPONSE_BODY_TRUNCATED_ATTR])

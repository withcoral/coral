...
`properties(startNode(relationship)).field` are normalized to ordinary property
references before IR construction when the index is a string literal or bound
scalar string parameter. This keeps the map-compatible surface useful while
preserving the core invariant that Coral does not materialize dynamic map
values.
...
- parser-accepted static list comprehensions such as `[k IN keys(node)]`,
  ...
  therefore composes with property predicates, projection, ordering, and
  aggregate targets without adding a dynamic map representation.
...
- `EXISTS { MATCH ... }` and compact `EXISTS { pattern WHERE ... }` lower to SQL
  ...
  semantics. Correlated count projections and hidden `ORDER BY` keys precompute
  grouped rows and therefore require the distinct target to be renderable from
  scoped inner rows. `OPTIONAL MATCH`, `WITH`, multi-item distinct counted
  returns, `RETURN` over graph objects, return pagination inside scoped
  subqueries, and `UNION` inside scoped subqueries still require staged planning
  and are rejected before SQL lowering;

...
Modules: `cli.rs` (argument parsing), `runner.rs` (command orchestration), `host.rs` (terminal I/O and prompt traits), `services.rs` (gRPC service wiring), `onboard.rs` (interactive setup flow), `source_ops.rs` (source helpers), and a thin `main.rs` entry point. Delegates all business logic to `coral-client`.

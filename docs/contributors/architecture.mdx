...
Intentionally thin — `main.rs` owns command parsing and delegates business logic to `coral-client`.
For black-box CLI tests, contributors can enable the hidden `--server` override with `cargo test -p coral-cli --features cli-test-server`.
...
| Owns               | Details                                           |
| ------------------ | ------------------------------------------------- |
| Local transport    | `ClientBuilder` starts a local server or connects to a remote endpoint |
...
| Source management   | Install, list, validate, remove (`sources/`)         |

...
Intentionally thin — `main.rs` owns command parsing, `bootstrap.rs` owns CLI bootstrap, and query/result helpers stay in `coral-client`.
For black-box CLI tests, contributors can enable the hidden `CORAL_ENDPOINT` bootstrap override with `cargo test -p coral-cli --features cli-test-server`.

...
- **`coral`** — teaches your agent the capability-first MCP workflow for answering data questions through Coral, including `search`, `describe`, Code Mode `exec`/`wait`, typed refs, and `coral.sql.query`.
- **`coral-create-source-spec`** — guides your agent through authoring or repairing a custom source spec YAML for `coral source add --file`, or adapting it into an active bundled SourceSpec.
...
<Note>
  Active [bundled SourceSpec](/reference/bundled-sources) specs are not copied
  into the config directory. Coral resolves them from the current binary when
  you validate or query a bundled source, so upgrades pick up newer bundled
  SourceSpecs without re-adding the source.
</Note>

...
- `telemetry/traces.sqlite3` for local OpenTelemetry spans when `[otel].local_trace_store` is enabled

To capture local traces without running an external OpenTelemetry store, add:

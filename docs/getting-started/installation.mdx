...
<Tabs>
  ...
  <Tab title="Build from source">
    Requires [Rust and Cargo](https://www.rust-lang.org/tools/install) plus Node.js/npm for the embedded UI.
    ...
    To build only the CLI, without the embedded UI:

    ```shellscript
    cargo install --path crates/coral-cli --locked --no-default-features
    ```
  </Tab>
</Tabs>

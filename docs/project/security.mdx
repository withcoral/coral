...
through `coral.sql.query`, not as a separate MCP tool. Secret
values are resolved by the local app at materialization or invocation time and
are not returned through discovery, MCP results, Code Mode globals, logs, or
diagnostics.

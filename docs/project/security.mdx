...
MCP tool surface: `search`, `describe`, `exec`, `wait`, and `feedback`.
SQL is available through the CLI and inside Code Mode
through `coral.sql.query`, not as a separate MCP tool. Coral-managed source
secret inputs are resolved by the local app at materialization or invocation
time and are not returned through discovery, Code Mode globals, logs, or
diagnostics. Provider-originated response data, including REST response bodies
and headers, is returned as untrusted provider data and may contain whatever the
provider sent.
...
- SQL execution is configured as read-only: DDL, DML, and multiple statements
  are disabled.
- Provider output is treated as untrusted data. Coral may shape it into SQL
  rows, TypeScript results, or MCP responses, but provider content is not used
  as an authorization decision.
- Query, mutation, and action capabilities are callable by default; source
  manifests and provider auth determine what an installed source can do.
- Source and workspace names are validated before they are used in local state
  paths.
- MCP clients can discover installed source exports and run Code Mode, but they
  do not receive raw stored secret values through discovery results, resources,
...
- A connected MCP client can discover installed source exports and run Code
  Mode against the app services available in your workspace.
...
- A custom source spec can direct Coral to acquire provider snapshots, make
  provider requests through supported interfaces, or read local file locations
  declared by that spec. Only install source specs you trust.
...
- Use `coral source list` or `coral source info <NAME>` to inspect installed
  interfaces and whether a source uses `keychain` or `file (plaintext)` secret
  storage.
- Review custom source specs before installing them, especially `base_url`,
  `auth`, and file `location` fields.
- Coral only sends source auth to OpenAPI or GraphQL descriptor/schema
  acquisition URLs when that URL is same-origin with the runtime provider URL.

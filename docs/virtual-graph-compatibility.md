...
| `RETURN` / `ORDER BY` / `WHERE` scalar expressions | Supported foundation | Arithmetic `+`, `-`, `*`, `/`, and `%`, searched `CASE WHEN ... THEN ... ELSE ... END`, plus `coalesce(...)`, `toString(...)`, `toInteger(...)`, `toFloat(...)`, `toBoolean(...)`, `toLower(...)`, `toUpper(...)`, `trim(...)`, `lTrim(...)`, `rTrim(...)`, and `replace(...)` over graph properties, scalar literals, scalar parameters, and nested supported scalar expressions. CASE `WHEN` predicates support property/scalar comparisons, `IN` literal lists, `exists(property)`, null checks, and `AND`/`OR`/`NOT`; `id()`, `elementId()`, `labels()`, `type()`, label predicates, bound-variable null checks, `XOR`, and optional-variable references are not supported inside CASE yet. Exponentiation and generic `CASE expr WHEN ...` are not supported yet. |

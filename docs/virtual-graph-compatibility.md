...
| `RETURN sum/avg/min/max/median/stDev/stDevP(property)` | Supported foundation | Numeric aggregate projections over mapped properties; `median` lowers to DataFusion `MEDIAN`, `stDev` lowers to sample standard deviation, and `stDevP` lowers to population standard deviation |
| `RETURN sum/avg(DISTINCT property)` | Supported foundation | Distinct numeric aggregate projections are rendered through DataFusion `SUM(DISTINCT ...)` and `AVG(DISTINCT ...)` |
...
| Terminal `WITH` projections | Supported foundation | Terminal projection, alias filtering, ordering, skip, limit, final alias renaming, and pure `RETURN *` alias pass-through are supported without staging another `MATCH` |

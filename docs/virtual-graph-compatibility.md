...
| `RETURN` / `ORDER BY` / `WHERE` scalar expressions | Supported foundation | `coalesce(...)`, `toString(...)`, `toLower(...)`, and `toUpper(...)` over graph properties, scalar literals, scalar parameters, and nested supported scalar expressions; predicates support comparisons, string predicates with string literal RHS, and `IN` literal lists |

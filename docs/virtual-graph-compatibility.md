...
| `RETURN` / `ORDER BY` / `WHERE` scalar expressions | Supported foundation | Arithmetic `+`, `-`, `*`, `/`, and `%`, plus `coalesce(...)`, `toString(...)`, `toInteger(...)`, `toFloat(...)`, `toBoolean(...)`, `toLower(...)`, `toUpper(...)`, `trim(...)`, `lTrim(...)`, `rTrim(...)`, and `replace(...)` over graph properties, scalar literals, scalar parameters, and nested supported scalar expressions; predicates support comparisons, string predicates with string literal RHS, `IN` literal lists, and null checks. Exponentiation is not supported yet. |

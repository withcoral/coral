...
| GraphQL object variables and defaults | Rejected | The current variable API binds values inside supported scalar/list/name positions; whole `where` objects, property-condition objects, and GraphQL variable defaults need schema-level input coercion |

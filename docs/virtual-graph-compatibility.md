...
| `RETURN` / `ORDER BY` / `WHERE coalesce(...)` | Supported foundation | Scalar expression over graph properties, scalar literals, scalar parameters, and nested `coalesce`; predicates support comparisons, string predicates with string literal RHS, and `IN` literal lists |

...
| `RETURN` / `ORDER BY coalesce(...)` | Supported foundation | Scalar expression over graph properties, scalar literals, scalar parameters, and nested `coalesce`; predicate expression support is deferred |

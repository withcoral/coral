...
| `WITH` pass-through | Supported foundation | Transparent `WITH var, ...` and `WITH *` preserve bound graph variables; `WHERE` predicates over carried graph variables lower into the normal predicate tree |
...
| GraphQL parser | Supported foundation | Root-node query fields compile directly to shared IR with scalar property selections, root aliases, root-level named/inline fragments with `Query` type conditions, node-level `__typename`, named and inline fragments on matching node labels, `@include(if:)` / `@skip(if:)`, `where`, `orderBy`, `limit`, `offset`/`skip`, and `distinct`; typed GraphQL variables can bind scalar literals, scalar-list `in` values, enum/string names, booleans, and non-negative integer row modifiers through the explicit variable API, with scalar/list/default enum values used when runtime variables are omitted; `where`, nested `where`, and `relationshipWhere` support property predicates including `matches`/`regex`, plus `and`, `or`, binary `xor`, and `not` boolean composition; relationship fields named `out_TYPE(to: Label)`, `in_TYPE(from: Label)`, or `any_TYPE(label: Label)` compile to normal graph patterns; `_edge { ... }` projects relationship properties |
| GraphQL unsupported generated-client features | Rejected | Fragment definition directives, operation directives, unknown directives, mismatched fragment type conditions, fragment cycles, conflicting response aliases, and edge-level `__typename` are rejected before SQL lowering |
| GraphQL object variables and object defaults | Rejected | The current variable API binds values inside supported scalar/list/name positions; scalar, list, enum/name, boolean, and integer defaults are supported, while whole `where` objects and property-condition objects still need schema-level input coercion |

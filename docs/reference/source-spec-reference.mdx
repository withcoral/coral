...
| `CustomAuth` | Dispatch request signing to a runtime-registered authenticator (for schemes that must observe the built request, such as AWS SigV4). |

All three resolve their inputs from declared source inputs (`{{input.KEY}}` templates or `from: input`). Auth is source-scoped — it does not see SQL filters or pagination state.
...
The `authenticator` key names a runtime-registered `RequestAuthenticator`. All other fields under `auth` are passed through as the authenticator's config. Custom authenticators receive the fully built `reqwest::Request` (method, URL, body, declared headers) before it is sent — use this type when signing depends on request contents.

##### Coral CLI: `aws_sigv4`

Signs outgoing requests with AWS Signature Version 4. The Coral CLI registers this authenticator from the `coral-auth-aws` crate for its local server. Useful for querying AWS monitoring services via CloudWatch, CloudTrail, or any other SigV4-protected API.
...
auth:
  ...
  session_token: "{{input.AWS_SESSION_TOKEN}}"   # optional, for STS/role-assumed credentials
```
...
##### Registering additional custom authenticators

Custom authenticators are Rust types that implement the `coral_engine::RequestAuthenticator` trait and are registered by the embedding process. End users of a Coral distribution get the set the embedder chose to ship; adding new ones requires building a custom binary.
...
pub trait RequestAuthenticator: Send + Sync + std::fmt::Debug {
    ...
    fn validate(
        ...
        auth: &coral_spec::CustomAuthSpec,
        resolved_inputs: &BTreeMap<String, String>,
    ) -> Result<(), RequestAuthenticatorError> { Ok(()) }
}
```

Contract:

- `name()` returns the stable dispatch key referenced by `auth.authenticator` in manifests.
- `authenticate()` inspects the built request and returns headers to attach. It runs per-request, after all other declared headers are in place, and has access to source inputs but not to SQL filters.
- `validate()` runs at source-registration time against resolved inputs. Use it to fail fast on missing credentials before any request is issued. Default returns `Ok(())`.
- Config fields declared alongside `authenticator:` in the manifest arrive as `auth.config` (a `serde_json::Map`). Deserialize into your own struct with `serde_json::from_value`.

Register authenticators by providing engine extensions when starting an embedded local server:

```rust
use std::sync::Arc;
use coral_auth_aws::AwsSigV4Authenticator;
use coral_client::local::{EngineExtensions, EngineExtensionsProvider, QuerySource, ServerBuilder};

#[derive(Debug)]
struct MyExtensions;

impl EngineExtensionsProvider for MyExtensions {
    fn extensions_for(&self, _selected_sources: &[QuerySource]) -> EngineExtensions {
        ...
        extensions.request_authenticators.insert(
            "aws_sigv4".to_string(),
            Arc::new(AwsSigV4Authenticator),
        );
        // extensions.request_authenticators.insert("my_custom".to_string(), Arc::new(MyCustomAuthenticator));
        extensions
    }
}

let server = ServerBuilder::new()
    .with_engine_extensions_provider(Arc::new(MyExtensions))
    .start()
    .await?;
```

Manifests then reference the registered authenticator by its `name()` value:

```yaml
auth:
  type: CustomAuth
  authenticator: my_custom
  # ...config consumed by MyCustomAuthenticator
```

An unregistered authenticator name fails source registration with a clear error — no silent fallback.

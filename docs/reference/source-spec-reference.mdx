...
Converts a numeric or string epoch value into microseconds for a `Timestamp` column.
...
Builds a string by substituting `{key}` placeholders with evaluated expressions. Supports a `{key|remove:X}` modifier that strips all occurrences of `X` from the value before substitution.
...
- name: permalink
  ...
  expr:
    kind: template
    template: "https://example.com/archives/{channel}/p{ts_id|remove:.}"
    values:
      channel:
        kind: from_filter
        key: channel

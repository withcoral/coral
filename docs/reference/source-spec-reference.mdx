...
onboarding:
  instructions: >-
    Use my_source.me to identify the authenticated account before
...
| `onboarding.instructions` | Optional source-authored guidance for agents to discover source context, identity, auth scope, or provider-specific query semantics. Surfaced through `coral.sources` and MCP guide context. |
...
### Inspecting source metadata and configured inputs

At runtime, installed source metadata is surfaced through `coral.sources`.
Use this when an agent or script needs source-level context before probing
provider identity, auth scope, or source-specific search semantics.

`coral.sources` includes these columns:

| Column | Description |
| ------ | ----------- |
| `schema_name` | Source schema name |
| `description` | Manifest-authored source description |
| `onboarding_instructions` | Optional manifest-authored agent guidance |

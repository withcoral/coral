...
`web` Dynamic Client Registration application type. Use
`initial_access_token_input` when the registration endpoint requires an initial
access token; Coral collects that value during OAuth setup and sends it as
bearer auth to the registration endpoint. Set

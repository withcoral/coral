...
To make a provider-native search function eligible for Universal Search
execution from `coral search` or the MCP `search` tool, add an explicit
`universal_search` block. Execution also requires the
`search_provider_fanout` runtime feature; without that feature, eligible
functions still appear as native search paths but are not executed
automatically. Execution is never inferred from `kind: search` alone. Exactly
one function argument must receive the search query, and every other argument
must declare a `default` value. If that argument declares a `values` enum, the
default must be one of those values:
...
Functions that need extra arguments without defaults should stay callable from
SQL and may still appear as native search paths, but Universal Search will not

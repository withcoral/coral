...
| `Json`      | UTF-8 string containing JSON. Hint that the column is queryable with `json_get`, `->`, `->>`, etc. |
...
Available: `json_get`, `json_get_str`, `json_get_int`, `json_get_float`, `json_get_bool`, `json_get_array`, `json_get_json`, `json_contains`, `json_length`, `json_as_text`, plus the `->` and `->>` operators.

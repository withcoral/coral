...
Available: `json_get`, `json_get_str`, `json_get_int`, `json_get_float`, `json_get_bool`, `json_get_array`, `json_get_json`, `json_contains`, `json_length`, `json_as_text`, plus the `->` and `->>` operators. For detailed function behavior, see the [datafusion-functions-json reference](https://github.com/datafusion-contrib/datafusion-functions-json/blob/main/README.md).

`->` returns a JSON value that you can continue traversing or cast to a typed SQL value. `->>` returns text.

Use plain object keys or array indexes with these functions and operators. They do not accept JSONPath syntax such as `$.browser`. For example, `properties->>'browser'` looks up the `browser` key. Some sources, including PostHog, use literal keys that start with `$`, so `properties->>'$browser'` looks up the exact key name `$browser`.

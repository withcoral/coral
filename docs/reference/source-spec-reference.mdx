...
Choose client settings from the provider's OAuth app and token endpoint
requirements. Use `pkce: required` for public clients when the provider
supports it. Use `pkce: disabled` only for providers that do not support PKCE.
...
Rendered OAuth URL fields must use `https`; plain `http` is accepted only for
loopback hosts such as `localhost`, `127.0.0.1`, or `::1` for local development
and tests. Device-code verification URLs returned by the provider are checked
with the same rule before the CLI opens them.
...
When the provider requires pre-registered redirect URIs with exact ports, use
`redirect_uri_port_mode: fixed` with a non-zero port and tell users to register
that exact URI in the OAuth app.

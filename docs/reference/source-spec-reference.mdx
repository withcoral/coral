...
When a column is derived from a nested object, Coral uses `__` to join path
segments in the SQL column name.

- `user__login` maps to `user.login`
- `head__repo__full_name` maps to `head.repo.full_name`

This keeps nested response structure visible in SQL while still exposing a flat
column surface.

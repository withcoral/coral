...
| Field         | Description                                        |
| ------------- | -------------------------------------------------- |
| `name`        | Source name, also used as the SQL schema name for DSL v3 and single-surface DSL v4 sources |
...
table functions. For DSL v3 MCP sources, the manifest remains the contract for
SQL names, filters, arguments, response shape, and columns; Coral does not
introspect MCP tools at query time.

DSL v4 preview manifests can also declare MCP surfaces. In v4, Coral discovers
the MCP server's tool catalog during `coral source add`, materializes generated
projections, and then queries only those installed artifacts. Each surface has
an optional `namespace` field that becomes the SQL schema for generated
projections. If omitted, the namespace defaults to the source `name` for a
single-surface source, or `{source.name}_{surface.id}` for a multi-surface
source. Two surfaces in the same source cannot declare the same namespace;
`coral source add` rejects that manifest. Re-add the source when the server's

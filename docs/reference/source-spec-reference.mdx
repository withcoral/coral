...
To let Universal Search execute a provider-native search function directly from
`coral search` or the MCP `search` tool, add an explicit `universal_search`
block. Execution is never inferred from `kind: search` alone. Exactly one
function argument must receive the search query, and every other argument must
declare a `default` value. If that argument declares a `values` enum, the

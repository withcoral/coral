...
| `CustomAuth` | Dispatch request signing to a built-in Coral authenticator (for schemes that must observe the built request, such as AWS SigV4). |
...
The `authenticator` key selects a built-in Coral authenticator. All other fields under `auth` are passed through as that authenticator's config. Use this type when signing depends on the final request contents.

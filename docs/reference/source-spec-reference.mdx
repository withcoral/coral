...
Coral source specs currently use `dsl_version: 3`. Manifests declare a top-level
`backend`, optional `inputs`, and authored `tables` or `functions`.
...
| `dsl_version` | Coral source spec DSL version: `3`                 |

...
tables:
  - name: messages
    ...
    guide: Query directly; no required filters.
    source:
...
- `guide` is optional. Coral surfaces it through `coral.tables.guide`; when you
  omit it, Coral generates fallback guidance from the table's declared filters
  and `fetch_limit_default`.

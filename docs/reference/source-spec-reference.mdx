...
- `oauth`: run an OAuth authorization-code flow with a loopback callback URI and
  store the returned access token in the same source secret.

Use `type: oauth` when the provider should issue the source secret through a
browser-based OAuth setup instead of asking the user to paste a token. If you
want to support both flows, list the OAuth method first and add a
...
OAuth methods require `flow.type: authorization_code` and an explicit `pkce` of
`required` or `disabled`. Public clients can declare `client.id.default`,
`client.id.input`, or both; an interactive `input` value overrides `default`.
OAuth methods whose token endpoint requires client-secret authentication must
prompt for both OAuth client values: set `client.id.input`,
`client.secret.input`, and a `client.secret.transport` of `basic_auth` or
`request_body`. Client secrets are sent only to the token endpoint and are never
...
OAuth currently supports loopback HTTP callback URIs:

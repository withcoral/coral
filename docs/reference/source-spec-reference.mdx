...
| `max_concurrency`    | integer         | —               | Maximum concurrent dependent-join HTTP calls for this source                |
...
Coral can mark HTTP table filters as safe for future dependent predicate
pushdown. This metadata is table-local: it says a filter may be supplied from a
runtime join key, not that the table depends on a specific source pair.

V1 capability metadata is deliberately narrow:

| Field | Scope | Type | Default | Description |
| ----- | ----- | ---- | ------- | ----------- |
| `filters[].bindable` | filter | boolean | `false` | Allows the filter to be supplied from a runtime join key |
| `filters[].wire_type` | filter | string | `string` | Runtime binding serialization; V1 supports only `string` |
| `filters[].max_bindings` | filter | integer | — | Cap for distinct values when this single filter is used alone |
| `search_index` | table | boolean | `false` | Marks filtered results as ranked, capped, fuzzy, or otherwise incomplete |
| `dependent_join.max_bindings` | table | integer | — | Cap for composite binding tuple fanout, and fallback single-filter cap |
| `dependent_join.max_resolver_rows` | table | integer | — | Cap for resolver-side rows buffered by dependent join execution |
| `dependent_join.max_rows_per_binding` | table | integer | — | Cap for rows returned by one dependent fetch |

Bindable filters must use `mode: equality`. Search-index-backed tables cannot
declare bindable filters because their filtered result sets are not complete
enough to preserve SQL join correctness.

...
Coral is extensible through source specs — YAML files that describe how to connect to an API or read a local dataset. If the system you need is not in the [bundled source set](/reference/bundled-sources), you can write a spec and add it from a file:
...
<<<<<<< HEAD
||||||| parent of d6cce13 (chore: update docs, to reflect changes to cli)
Then import it:
...
>>>>>>> d6cce13 (chore: update docs, to reflect changes to cli)
Notes:
...
When the spec is added, Coral prompts for those values interactively.

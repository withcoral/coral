...
- `test_queries` may substitute installed source variables using `{{input.VARIABLE_KEY}}`
  tokens (same syntax as HTTP path and header templates). Secrets are not substituted.

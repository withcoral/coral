...
`access_token_scheme` controls how a retrieved access token is stored as the
secret value. The default, `raw`, stores the token unchanged. Set
`access_token_scheme: bearer` when the provider requires the `Bearer` scheme on
OAuth tokens but expects other credentials (such as a pasted API key) to be sent
verbatim: Coral then stores the token as `Bearer <token>` so a single secret
input can be read with a plain `from: input` header and still send the correct
`Authorization` value for both credential kinds. Only OAuth-retrieved tokens are
formatted; values supplied through `source_config` are stored as entered.
...
By default, OAuth stores the token response `access_token` as the source secret
value. If that raw token must be sent as `Authorization: Bearer <token>`, use
`from: bearer` with that input key. If one secret must support both pasted raw
API-key headers and OAuth bearer tokens, set `access_token_scheme: bearer` on
the OAuth method and send the secret with `from: input`. Use separate optional
secrets plus `from: one_of` only when the provider really has separate stored
credential slots.

...
credential retrieval choices. Coral surfaces this metadata through source
discovery and source-info responses, but the CLI still collects secret values
through the normal environment variable or prompt path. This does not replace
the top-level `auth` block: `credential.methods` describes install-time
credential retrieval options, while `auth` is runtime request authentication.
...
`redirect_uri`, but does not run the OAuth flow yet. When users provide their
own OAuth client ID for a future OAuth retrieval flow, they must configure that
exact redirect URI in the OAuth app.
...
`scope=a b`, and `delimiter: comma` represents `scope=a,b`. OAuth token
exchange, refresh-token storage, and automatic token refresh are not
implemented yet.

...
Signs outgoing requests with AWS Signature Version 4. Ships in the `coral-auth-aws` crate. Useful for querying AWS monitoring services via CloudWatch, CloudTrail, or any other SigV4-protected API.
...
Register authenticators by implementing `QueryRuntimeProvider::engine_extensions()` in your embedding:

```rust
use std::collections::HashMap;
use std::sync::Arc;
use coral_engine::{EngineExtensions, QueryRuntimeProvider, RequestAuthenticator};

fn engine_extensions(&self) -> EngineExtensions {
    let mut authenticators: HashMap<String, Arc<dyn RequestAuthenticator>> = HashMap::new();
    authenticators.insert(
        "aws_sigv4".to_string(),
        Arc::new(coral_auth_aws::AwsSigV4Authenticator),
    );
    // authenticators.insert("my_custom".to_string(), Arc::new(MyCustomAuthenticator));

    EngineExtensions {
        request_authenticators: authenticators,

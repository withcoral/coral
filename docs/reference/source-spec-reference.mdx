...
tries the values in order and uses the first one that has been configured. Put
the credential users should prefer first. When a choice references a secret
input, Coral requires at least one of those secrets to be supplied or already
stored before the source can be installed or updated — unless another choice
always provides a value on its own (such as a literal), in which case auth never
depends on a secret and none is required:

...
- `audience`: optional provider-specific object that narrows the target API,
  ...
  outbound request host is that host or one of its subdomains.
...
type and setup needed to create one. `coral source add --file` accepts a
multi-document YAML file with one source spec document plus `kind: identity`
documents separated by `---`; it installs those identity specs globally, then
prompts for a compatible existing user-owned identity (or creates one) and
records the local user's selection for the surface separately from the
workspace source config. Re-running `coral source add --file` with the same
manifest replaces that selection.

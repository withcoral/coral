...
Use plain object keys or array indexes with these functions and operators, not JSONPath syntax. For example, `properties->>'browser'` looks up the `browser` key, while `properties->>'$browser'` is only for a literal key named `$browser`.

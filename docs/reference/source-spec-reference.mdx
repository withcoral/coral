...
Secret inputs can also declare `credential.methods` to describe how Coral can
retrieve the secret during interactive install. This does not replace the
top-level `auth` block: `credential.methods` is install-time credential
retrieval, while `auth` is runtime request authentication.
...
used. OAuth currently supports authorization-code flow with a fixed loopback
HTTP callback URI:
`http://127.0.0.1:<port>/<path>` or
`http://localhost:<port>/<path>`. Coral binds the exact host, port, and path
from `redirect_uri`; if that port is busy, install fails instead of choosing a
random port. When users provide their own OAuth client ID, they must configure
that exact redirect URI in the OAuth app.

`pkce` is either `required` or `disabled`. Public clients can declare
`client.id.default`, `client.id.input`, or both; an interactive `input` value
overrides `default`. Confidential clients add `client.secret.input` and a
required `transport` of `basic_auth` or `request_body`. Client secrets are sent
only to the token endpoint and are never included in the authorization URL.

Scopes are declared under `scopes.scope`; `delimiter: space` produces
`scope=a b`, and `delimiter: comma` produces `scope=a,b`. Coral may store

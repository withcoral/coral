...
function arguments, state, or inline defaults. Rendered OAuth provider endpoint
URLs must use `https`; plain `http` is accepted only for loopback hosts such as
`localhost`, `127.0.0.1`, or `::1` for local development and tests. Device-code
verification URLs returned by the provider are checked with the same rule before
the CLI opens them.

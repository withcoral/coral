...
credential retrieval choices. When `credential` is omitted, Coral collects the
secret the same way as `type: source_config`: from an environment variable by
default, or from an interactive prompt when `coral source add --interactive` is
used. `credential.methods` only controls how the secret is collected; runtime
request authentication is still defined by `auth` or request headers that
reference the input.

Use `type: oauth` when the provider should issue the source secret through a
browser-based OAuth authorization-code flow instead of asking the user to paste a
token. The OAuth method populates the same secret input that `auth` references at
runtime, so the request configuration does not change after setup. If you want to
support both flows, list the OAuth method first and add a `source_config`
fallback for users who already have a token.
...
`required` or `disabled`. Public clients can declare
`client.id.default`, `client.id.input`, or both; an interactive `input` value
overrides `default`. Confidential clients must declare `client.id.input`,
`client.secret.input`, and a `client.secret.transport` of `basic_auth` or
`request_body`; they may also include `client.id.default`. Client secrets are
sent only to the token endpoint and are never included in the authorization URL.

OAuth `client.id.input` and `client.secret.input` are method-specific credential
retrieval inputs, not top-level entries in the manifest `inputs` map.

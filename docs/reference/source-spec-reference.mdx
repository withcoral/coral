...
results before producing joined output. The caps above bound the number of
distinct binding tuples, resolver rows, and rows returned by each dependent
fetch; they do not cap total joined output rows.

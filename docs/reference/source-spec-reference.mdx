...
projections, and then queries only those installed artifacts. Re-add the source
when the server's tool catalog changes. If generated MCP projection names
collide with generated REST projection names in the same source, Coral keeps the
REST projection name and renames the MCP projection with a stable collision
suffix.

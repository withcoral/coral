...
Before installing, Coral prints the outbound network hosts declared by the
source. Host values are resolved with the variables you provide, so a
self-hosted base URL such as `GITHUB_API_BASE=https://github.example.com/api/v3`
is shown before the source is connected. In interactive mode, Coral asks you to
confirm those hosts before collecting secrets or starting OAuth.
...
Coral resolves and prints the source's declared outbound hosts before installing it.
...
When onboarding connects a source, Coral resolves and shows the source's
outbound hosts before asking for confirmation.

...
coral --disable-episodes mcp-stdio
```
...
coral features disable episodes
```
...
When the `episodes` feature is enabled, the server also exposes a non-read-only `create_episode` tool. Agents can call it once at the start of a task, then pass the returned `episode_id` on follow-up Coral tool calls so Coral can attribute those calls to the task intent. Episode records are stored locally; indexing and retrieval over those records are still in progress.

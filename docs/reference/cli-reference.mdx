...
Before installing or updating credentials, Coral prints the outbound network
hosts declared by the source. Host values are resolved with the variables you
provide, so a self-hosted base URL such as
`GITHUB_API_BASE=https://github.example.com/api/v3` is shown before setup
continues. In interactive mode, Coral asks you to confirm those hosts before
collecting secrets or starting OAuth.
...
If the source is already installed, running this command again replaces its
stored credentials with the new values you provide. After setup, it prints the
discovered tables and runs any optional top-level `test_queries` declared in the
source spec to validate the connection. Any validation issue is reported as a
warning here so the source remains installed and re-runnable.
...
Coral resolves and prints the source's declared outbound hosts before setup continues.
...
When onboarding connects a source or updates credentials for an installed
source, Coral resolves and shows the source's outbound hosts before asking for
confirmation.

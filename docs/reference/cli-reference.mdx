...
For auth alternatives declared with `from: one_of`, at least one referenced
secret must be supplied or already stored unless another choice always supplies
a value on its own.

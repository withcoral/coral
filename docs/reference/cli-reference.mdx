...
registered SQL surfaces. Provider-backed generated schemas include the source
key and interface id, for example `<source>_<interface>.<table>`:

```shellscript
coral sql "SHOW TABLES"
coral sql "SHOW COLUMNS FROM datadog_rest_v1.hosts"
coral sql "DESCRIBE datadog_rest_v1.hosts"
```
  ...
  "source_status": [
    {
      "source_id": "src_demo",
      "source_key": "demo",
      "capability_id": "source/src_demo/interface/rest/operation/list_items",
      "binding_ref": "typescript:demo.rest.items.listItems",
      "full_path": "tools.demo.rest.items.listItems",
      "ok": true,
      "complete": true,
      "partial": false,
      "error": null
...
`value` is the provider value. `envelope` carries provider response metadata
and, for compatibility with existing CLI consumers, may include the provider
payload fields such as REST `body`, GraphQL `data`, or MCP
`structured_content`. `complete`, `errors`, and `source_status` are kept
for compatibility with existing structured-output consumers, and
`source_status` carries the invoked binding provenance plus per-source
ok/complete/partial/error status. Provider or validation failures print the wrapper
with `ok: false` and exit non-zero. Query, mutation, and action capabilities
are callable by default.
...
The output includes the installed interface ids and the source's secret storage
route: `keychain` or `file (plaintext)`.

### `coral source info <NAME>`

Show metadata for a source: whether it is installed, its origin, interface ids,
version, description, and inputs. Works for active bundled SourceSpecs and
imported sources installed via `coral source add --file`.
...
Pass `--interfaces` to install only specific provider interfaces from a
multi-interface source. The value is a comma-separated list of interface ids.
Coral filters the source spec before collecting inputs and materializing
artifacts, so credentials for unselected interfaces are not required.

```shellscript
coral source add slack --interfaces rest
coral source add --interactive slack --interfaces mcp
```
...
coral source add --file ./multi-provider.yaml --interfaces rest
```
...
| Tool | `describe` | Describe refs with JSON input/output schemas       |
| Tool | `exec`     | Run Code Mode and return `{ run, result, output, cursor }` |
| Tool | `wait`     | Wait for new Code Mode output by cursor, or terminate a run |

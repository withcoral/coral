...
Expose the local Coral runtime as a streamable HTTP MCP server.

```shellscript
coral mcp-http --host 0.0.0.0 --port 8080 --path /mcp
```

This command is intended for long-running service deployments. For local
desktop clients, prefer `coral mcp-stdio` unless the client specifically needs
an HTTP MCP URL.
...
| `--path` | `/mcp`    | HTTP path where MCP is served       |

This server exposes the same tools and resources as `coral mcp-stdio`.

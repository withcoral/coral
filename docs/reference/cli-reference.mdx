...
coral --disable-sql-codemode mcp-stdio
...
| `-f, --file` | path            |         | Execute a semicolon-delimited SQL file     |
...
When `--file` is used, Coral executes statements sequentially in one private
batch session and stops on the first error. Statements in SQL files must be
separated with semicolons. Batch mode supports session-local temporary tables
created with `CREATE TEMP TABLE ... AS SELECT ...`; normal inline
`coral sql "<SQL>"` remains read-only and does not allow DDL or DML.

Batch JSON output is an array of result-set objects:
...
Statements that only create batch-local temporary tables do not appear in the
result-set array.

## `coral codemode sql`

Execute one SQL codemode body against Coral and exit.

```shellscript
coral codemode sql --code 'SELECT 1 AS value'
coral codemode sql --file ./analysis.sql --format json
```

SQL codemode executes semicolon-delimited statements sequentially in one private
query session. It accepts read-only query statements plus
`CREATE TEMP TABLE ... AS SELECT` for session-local temporary tables. Temporary
table creation statements do not appear in the result-set array.

The CLI command is explicit and does not require the `sql_codemode` runtime
feature flag. That feature only controls whether MCP exposes the
`sql_codemode` tool.
...
| Option     | Type              | Default | Description                                |
| ---------- | ----------------- | ------- | ------------------------------------------ |
| `--code`   | string            |         | SQL statements to execute                  |
...
Use catalog tables before querying:
...
```shellscript
coral features enable sql_codemode
...
```shellscript
coral features disable sql_codemode
...
| Tool     | `sql_codemode`   | Optional SQL codemode execution in one private query session |

...
Use `table_function` to expose the recipe through SQL. Add `mcp` to expose it as an MCP tool. Recipe MCP tools are exposed with a `recipe_` prefix, so `mcp.name: open_prs` becomes the MCP tool `recipe_open_prs`.

...
Manage recipes. A recipe is a YAML artifact that defines a reusable, typed query and can publish it as SQL under `recipes.*`; bundled sources may also ship source-owned recipes.
...
publish:
  ...
  - mcp_tool: my_review_queue
```

Use `table_function` to expose the recipe through SQL. Use `mcp_tool` to expose it as an MCP tool once the MCP server refreshes its tool list.
...
The output is inventory-only and does not re-run runtime validation, so it stays fast. The `Origin` column shows `user` for recipes installed with `coral recipe add --file` and `bundled` for recipes shipped with an installed bundled source. For user-installed recipes, Coral shows the result columns cached during the last successful `coral recipe add --file`.

...
Bundled installs keep the source identity plus your configured variables and secrets, then resolve the bundled manifest from the current Coral binary at runtime.
...
Imported installs persist the manifest snapshot under Coral's config directory because that source spec is user-owned.

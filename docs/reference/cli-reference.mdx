...
These flags accept the feature keys reported by `coral features list`. They do
not edit `config.toml`. Use `coral features enable <FEATURE>` or `coral
features disable <FEATURE>` to persist a preference.

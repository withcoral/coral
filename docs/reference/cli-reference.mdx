...
Benchmark episode mode can additionally expose `search_trajectory`, an experimental local trajectory-memory lookup that returns prior exact-intent SQL procedure steps only after query and ordered-path consensus pass. It never returns cached rows.

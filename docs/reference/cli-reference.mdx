...
```shellscript
coral mcp-stdio [--enable-feedback]
```
...
| Tool     | `feedback`       | Store blocked-agent feedback when `--enable-feedback` is set |
| Resource | `coral://guide`  | Database workflow guide for agents |
| Resource | `coral://tables` | Database table summaries         |

`sql` returns structured `{ columns, rows, row_count }` results and accepts optional bound `params` as either an array for `$1` placeholders or an object for named placeholders.

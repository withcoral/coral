...
Find likely Coral catalog items, columns, filters, observed values, native search paths, and, when enabled, executed provider search rows.

```shellscript
coral search "payments-api timeout"
coral search --limit 25 "payments-api timeout"
coral search --json "payments-api timeout"
```

The text output is compact and shows each result's type, name, and most useful payload detail. A table or column result points to a SQL surface to query, a native search path gives a table-function call shape, an observed value shows data seen in earlier local SQL output, and a provider search row contains executed provider-search data when the `search_provider_fanout` feature is enabled. Run `coral sql` with the returned reference or example call when you need current source rows.
...
| `--limit` | integer | `50`    | Maximum results to return, 1 to 100 |
...
`search`, `describe_table`, `list_columns`, and `coral://tables` can discover

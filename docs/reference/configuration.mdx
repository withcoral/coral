...
Global identity specs installed from `coral source add --file` bundles live
under `identity-specs/` in the same local state
...
principal. In OSS Coral the only user principal is `local`. To change that
user's selected

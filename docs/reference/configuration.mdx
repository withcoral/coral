...
| ------- | ------- | ----------- |
| `sql_codemode` | `false` | Experimental. Exposes the MCP `sql_codemode` tool when enabled. SQL codemode runs ordered read-only queries and session-local temporary-table CTAS statements in one private query session. |

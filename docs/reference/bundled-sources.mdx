...
Connect with Linear OAuth for guided setup, or paste a personal API
key with the Read permission (read-only access to all tables, and you
can optionally scope the key to specific teams).
Coral uses a built-in OAuth client by default; to use your own, enter
a client ID when prompted for LINEAR_OAUTH_CLIENT_ID. The app needs
the `read` scope and the redirect callback URL
`http://127.0.0.1:53682/oauth/callback`.
A pasted API key is not refreshed automatically; connect with guided
OAuth instead to store a refresh token and reconnect without re-pasting.
See [Linear API key docs](https://linear.app/docs/api-and-webhooks#api-keys)
and [Linear OAuth docs](https://linear.app/developers/oauth-2-0-authentication).

Credential methods:

- **Connect with Linear OAuth** — Use the default Linear OAuth app or provide your own client ID.
- **Paste a Linear API key** — Paste a personal API key with the Read permission.

...
AWS access key ID from an AWS access key pair with CloudWatch Logs
read permissions.

`AWS_SECRET_ACCESS_KEY` (required)

AWS secret access key from the same access key pair as
`AWS_ACCESS_KEY_ID`.
...
AWS access key ID from an AWS access key pair with the CloudWatch
read permissions needed for the tables you plan to query.

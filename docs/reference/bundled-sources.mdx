...
Connect with Linear OAuth, or provide a raw OAuth access token.
Create a Linear OAuth application with the `read` scope and the
redirect callback URL `http://127.0.0.1:53682/oauth/callback`.
Coral includes a default Linear OAuth client ID; during interactive
setup you can press Enter to use it or provide your own client ID.
Coral stores OAuth refresh token metadata, but automatic refresh is
not implemented yet; rerun
`coral source add linear --interactive` if the access token expires.

...
Coral installs [SourceSpecs](/reference/source-spec-reference) from active bundled entries, [community source specs](/reference/community-sources), or your own YAML files with `coral source add --file`. You can run SQL from the [CLI](/reference/cli-reference), and MCP agents can discover capabilities and run Code Mode through [Coral over MCP](/guides/use-coral-over-mcp). And everything is local; your data, credentials and usage history never leave your machine.
...
<Steps titleSize="h3">
  ...
  <Step title="Add your sources">
   Import a SourceSpec-backed API, upstream MCP, GraphQL, or file source into your workspace. For example:
  ...
  <Step title="Start querying">
    ...
    ```shellscript
    coral sql "SELECT id, text FROM local_messages.messages ORDER BY id"
    ```
...
Coral sits between your agents and your data sources: agents discover generated capabilities, run TypeScript bindings or SQL projections through Code Mode, and Coral translates those calls into provider API calls or file reads.
...
<CardGroup cols={2}>
  ...
  <Card title="Bundled sources" href="/reference/bundled-sources">
    Active SourceSpec entries compiled into the CLI
  </Card>

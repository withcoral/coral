...
112 scenarios, which is 6.93% of the full upstream scenario-definition inventory
and 8.66% of the read-candidate inventory.
...
The baseline currently contains 112 representative read-only scenarios:
...
- `CountSubquery`: 4 scenarios for `COUNT { ... }` projections and predicates,
  including scoped scalar `RETURN` validation and `RETURN DISTINCT scalar`
  row-counting.

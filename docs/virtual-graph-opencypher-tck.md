...
56 scenarios, which is 3.47% of the full upstream scenario-definition inventory
and 4.33% of the read-candidate inventory.
...
The baseline currently contains 56 representative read-only scenarios:
...
- `ScalarExpressions`: 15 scenarios for searched `CASE`, scalar-string
  `isEmpty(...)` predicates and projections, string case conversion, trim
  variants, `replace(...)`, `substring(...)`, `size(...)`, `left(...)`,
  `right(...)`, `reverse(...)`, numeric `abs(...)`, and adjacent rejection
  coverage.

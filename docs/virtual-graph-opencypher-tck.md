...
124 scenarios, which is 7.68% of the full upstream scenario-definition inventory
and 9.58% of the read-candidate inventory.
...
The baseline currently contains 124 representative read-only scenarios:
...
- `ListExpressions`: 12 scenarios for static `range(...)`, `split(...)`,
  ...
  `reduce(...)` folds, direct folded-list `ORDER BY` keys, plus legacy static
  `filter(...)` / `extract(...)` functions.
...
- `Aggregation`: 2 scenarios for grouped `_count` and exact

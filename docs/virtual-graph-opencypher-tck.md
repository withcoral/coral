...
551 scenarios, which is 34.12% of the full upstream scenario-definition inventory
and 42.58% of the read-candidate inventory.
...
The baseline currently contains 551 representative read-only scenarios:
...
- `With`: 30 scenarios for transparent scope filtering, scalar alias filters,
  ...
  with filtering, terminal `WITH *, expr AS alias RETURN *`, and explicit
  rejection coverage for aggregate `WITH` chains that require staged planning.
...
- `Temporal`: 8 scenarios for rejection-only temporal support boundaries,
  including `date(...)`, `datetime(...)`, `time(...)`, `duration(...)`,
  temporal map constructors, component access, arithmetic, and
  `duration.between(...)`.
...
The GraphQL baseline currently contains 94 representative read-only scenarios:
...
- `ErrorHandling`: 10 expected rejections for unknown graph-declared properties,
  `_id` string predicates, aggregate argument misuse, and empty `and`/`or`
  boolean arrays, which GraphQL filters reject instead of treating as vacuous
  truth or falsehood.

...
111 scenarios, which is 6.87% of the full upstream scenario-definition inventory
and 8.58% of the read-candidate inventory.
...
The baseline currently contains 111 representative read-only scenarios:
...
- `CountSubquery`: 3 scenarios for `COUNT { ... }` projections and predicates,
  including scoped scalar `RETURN` validation.

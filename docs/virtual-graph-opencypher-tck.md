...
420 scenarios, which is 26.01% of the full upstream scenario-definition inventory
and 32.46% of the read-candidate inventory.
...
The baseline currently contains 420 representative read-only scenarios:
...
- `Aggregation`: 20 scenarios for grouped counts, `count(*)`, numeric
...
- `With`: 24 scenarios for transparent scope filtering, scalar alias filters,
...
- `ListExpressions`: 36 scenarios for static `range(...)`, `split(...)`,
...
The GraphQL baseline currently contains 55 representative read-only scenarios:
...
- `RowModifiers`: 5 scenarios for root ordering, offset, limit,
  `ASCENDING`/`DESCENDING` and shorthand `orderBy` directions, and distinct
  projection.
...
- `IdentityFields`: 6 scenarios for `_id` / `_elementId` selection, `_id`
  ...
  filters.
- `NestedRelationships`: 5 scenarios for out/in/any relationship traversal with
  endpoint, relationship-property, and `_edge` projections.

...
96 scenarios, which is 5.94% of the full upstream scenario-definition inventory
and 7.42% of the read-candidate inventory.
...
The baseline currently contains 96 representative read-only scenarios:
...
- `ListExpressions`: 5 scenarios for static `range(...)`, `split(...)`,
  indexes, slices, endpoint functions, collection predicates, and list
  comprehensions, including `pow(...)` / `power(...)` map expressions.

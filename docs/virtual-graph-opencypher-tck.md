...
134 scenarios, which is 8.30% of the full upstream scenario-definition inventory
and 10.36% of the read-candidate inventory.
...
The baseline currently contains 134 representative read-only scenarios:
...
- `CollectSubquery`: 2 scenarios for `COLLECT { MATCH ... RETURN scalar }`
  ...
  scalar collection.
- `PatternComprehension`: 1 scenario for parser-recovered
  `[(pattern) WHERE predicate | scalar]` projection lists over correlated
  scoped relationship patterns.
...
- `MathematicalFunctions`: 6 scenarios for `exp(...)`, `log(...)` /

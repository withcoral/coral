...
Recorded HTTP body previews are capped by `http_body_max_bytes` bytes per body (`65536` by default). They are visible in local trace history only and are not exported to external OTLP collectors.

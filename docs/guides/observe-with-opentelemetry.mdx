...
When the experimental `episodes` feature is enabled and a tool call includes `episode_id`, Coral records `episode.id` on the MCP tool-call span and on query execution spans. The episode intent is stored in the local episode log, not on spans or metadata.

...
To disable local trace inspection:
...
HTTP bodies are kept out of OTLP export even when local body recording is enabled.

## Configuration reference

External OpenTelemetry export settings live under `[otel]` in `config.toml`. Coral-owned local trace inspection settings live under `[local_traces]`. There are no environment variable overrides except for [`CORAL_TRACE_PARENT`](#distributed-tracing).
...
| Key                   | Type    | Default | Description                                                                       |
| --------------------- | ------- | ------- | --------------------------------------------------------------------------------- |
| `enabled`             | boolean | `true`  | Enables local trace inspection without requiring an OTLP collector.               |

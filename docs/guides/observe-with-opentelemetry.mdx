...
| `log_filter`                   | string  | `coral_app=info,coral_engine=info,coral_telemetry.log_only=info`       | `tracing-subscriber` filter applied to logs (OTLP and stderr).                    |
| `trace_filter`                 | string  | `coral_app=trace,coral_client=trace,coral_mcp=trace,coral_engine=trace,coral_telemetry.trace_safe=trace,coral_engine::datafusion=off,coral.http.body=off,coral.mcp.body=off` | `tracing-subscriber` filter applied to OTLP spans.                                |
...
The `coral_telemetry.*` target names are reserved for Coral-owned internal events: `coral_telemetry.trace_safe` events may be attached to exported spans, while `coral_telemetry.log_only` events are never attached to exported spans, even if you configure a broader trace filter. Logs can still be exported to OTLP, so log-only events are not a place for secrets when external log export is enabled.

When trace history is enabled, raw SQL text is kept in local trace history for the Coral UI, but the `sql` span attribute is removed from OTLP trace exports.

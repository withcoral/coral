...
Coral can emit traces, logs, and metrics over OTLP/HTTP to any compatible backend (Grafana, Honeycomb, OpenObserve, Jaeger, Datadog Agent, etc.). Telemetry is **off by default**. External export activates when you set an `[otel]` section with an endpoint in your `config.toml`; local trace inspection can be enabled without an OTLP endpoint.
...
To inspect traces in Coral's local UI without running an external OpenTelemetry store, enable internal tracing:
...
Internal tracing captures the same query, source-loading, HTTP, and DataFusion spans that Coral can export over OTLP. It also includes DataFusion optimizer-rule spans by default so the local trace view can show the planning work that happened for a query. This local filter is separate from `trace_filter`, which only controls OTLP span export.

To include HTTP request and response bodies in local traces, opt in explicitly:
...
| `enable_internal_tracing`      | boolean | `false`                                                                | Enables local trace inspection without requiring an OTLP collector.               |
| `record_internal_http_bodies`  | boolean | `false`                                                                | Records HTTP request and response bodies in local traces when internal tracing is enabled. |
| `internal_http_body_max_bytes` | integer | `65536`                                                                | Maximum request or response body bytes recorded in a local trace.                 |

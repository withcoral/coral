...
HTTP request and response bodies are recorded in local traces by default, up to `internal_http_body_max_bytes` bytes per body. To disable local body recording:

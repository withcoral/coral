...
HTTP request and response bodies are recorded in local traces by default, up to `http_body_max_bytes` bytes per body (`65536` by default). To disable local body recording:
...
HTTP bodies are captured through Coral's local trace store, not as global span attributes. They are kept out of OTLP export and are not exposed to host-owned tracing subscribers even when local body recording is enabled.
...
| `record_http_bodies`  | boolean | `true`  | Records HTTP request and response bodies in local traces when local tracing is enabled. |

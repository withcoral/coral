...
HTTP request and response bodies are recorded in local traces by default, up to `internal_http_body_max_bytes` bytes per body (`65536` by default). To disable local body recording:
...
| `enable_internal_tracing`      | boolean | `true`                                                                 | Enables local trace inspection without requiring an OTLP collector.               |
| `internal_trace_retention_days` | integer | `7`                                                                    | Number of days of local trace spans to retain.                                    |
| `record_internal_http_bodies`  | boolean | `true`                                                                 | Records HTTP request and response bodies in local traces when internal tracing is enabled. |

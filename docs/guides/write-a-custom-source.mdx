...
When one secret needs to support either a complete pasted API-key header or an
OAuth access token, set `access_token_scheme: bearer` on the OAuth method and
send the stored secret with `from: input`. Coral stores OAuth-retrieved tokens
as `Bearer <token>` while leaving pasted `source_config` values unchanged. Use
separate optional secrets with `from: one_of` only when the provider really has
separate stored credential slots or different runtime auth locations.

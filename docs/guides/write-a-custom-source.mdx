...
- **Use `coral.tables` and `coral.columns`.** They show you exactly what Coral sees after adding a source — including required filters, column metadata, and SQL-ready identifier fields such as `sql_table_ref`, `sql_column_ref`, and `sql_qualified_column_ref`.

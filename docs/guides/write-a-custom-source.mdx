...
`from: one_of` to choose between them. Coral tries the values in order and uses
the first one that has been configured. Put the credential you want Coral to use
when both are present first. When a choice references a secret input, users must
provide at least one of those secrets before the source can be added, unless
another choice always supplies a value on its own.

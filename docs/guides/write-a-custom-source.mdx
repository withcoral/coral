...
2. Adds it with `coral source add --file ./my-source.yaml`
...
- **Use the system catalog.** `coral.tables` and `coral.columns` show you exactly what Coral sees after adding a source — including required filters and column metadata. See [System catalog](/concepts/system-catalog).

...
You can reference installed source variables in `test_queries` with `{{input.VARIABLE_KEY}}` (for example `billingAccounts/{{input.GCP_BILLING_ACCOUNT_ID}}`). Export the variable before `coral source add --file`.

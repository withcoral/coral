...
- When the provider supports OAuth Dynamic Client Registration, put `client.dynamic_registration.registration_url` under the OAuth method. Coral registers a client during interactive setup and uses the returned client ID for the authorization/device and token requests, so users do not need to create an OAuth app manually. Set `request_refresh_token_grant: true` only when the provider supports registering clients for refresh-token grants.

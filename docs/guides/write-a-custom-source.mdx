...
- OAuth provider endpoints (`authorization_url`, `device_authorization_url`, and `token_url`) must use `https`; use plain `http` only for loopback local development or test endpoints.

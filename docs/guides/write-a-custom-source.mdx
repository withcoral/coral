...
- When the provider supports OAuth Dynamic Client Registration, put `client.dynamic_registration.registration_url` under the OAuth method. Coral registers a native client during interactive setup and uses the returned client ID for the authorization/device and token requests, so users do not need to create an OAuth app manually. Source specs cannot configure a `web` Dynamic Client Registration application type. Set `request_refresh_token_grant: true` only when the provider supports registering clients for refresh-token grants.
...
- For OAuth-protected MCP Streamable HTTP servers, set `oauth.resource` to the MCP server URL when the authorization server requires OAuth resource indicators.
- When OAuth URLs need non-secret install-time configuration, such as a Microsoft tenant ID, declare that value as a `kind: variable` input and reference it with `{{input.KEY}}` in `endpoints.authorization_url`, `endpoints.token_url`, `endpoints.device_authorization_url`, `resource`, or `client.dynamic_registration.registration_url`. OAuth URL templates do not support secrets, filters, function arguments, state, or inline defaults.

...
A source spec tells Coral how to connect to an API or read a local dataset, what tables to expose, and what columns each table has. Once added, the source works exactly like a bundled one: same SQL, same `coral.sources`, `coral.tables`, `coral.columns` and `coral.inputs`, same MCP surface.
...
5. Inspects `coral.sources`, `coral.tables`, `coral.table_functions`, `coral.columns`, `coral.filters` and `coral.inputs` to check the shape
...
- **Use `coral.sources`, `coral.tables`, `coral.table_functions`, `coral.columns`, `coral.filters`, and `coral.inputs`.** They show you exactly what Coral sees after adding a source — including source context, required filters, search functions, and column metadata.

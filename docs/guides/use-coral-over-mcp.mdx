...
Coral does this because MCP uses JSON-RPC, and many clients — JavaScript/TypeScript `JSON.parse` and most MCP hosts — decode JSON numbers as IEEE-754 doubles. Values beyond 2^53 silently lose precision when read as a number (`-8504475857937456387` becomes `-8504475857937456000`), with no error raised. Encoding them as strings keeps the exact value intact end to end. Parsers that preserve integer precision, such as `jq`, handle bare numbers fine, but Coral cannot assume the client does.

The CLI is unaffected: `coral sql --format json` keeps emitting these types as JSON numbers, so existing shell pipelines over CLI output are unchanged.

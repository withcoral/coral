...
| Tool     | `list_tables`    | List tables for small catalogs or summarize large catalogs by schema |
...
For richer metadata, have the agent query `coral.tables`, `coral.columns`, and `coral.inputs` over the `sql` tool instead of relying only on `list_tables` or `coral://tables`. On large workspaces, `list_tables` returns schema counts and example table names to keep the first discovery call compact; `coral://tables` remains the full table-list resource.
...
The agent should call `list_tables` or query `coral.tables` and return the schemas from your installed sources. If `list_tables` returns `truncated: true`, the agent should follow the included SQL next steps to inspect one schema or table at a time.

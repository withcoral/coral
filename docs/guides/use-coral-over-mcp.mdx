...
The agent should call `list_tables`, `search_tables`, `describe_table`, `list_columns`, or query `coral.tables` and return the schemas from your installed sources. Large catalogs should be paged with `limit` and `offset`.

...
Coral can expose an experimental `create_episode` MCP tool behind the `episodes` feature flag. Enable it persistently:

```shellscript
coral features enable episodes
```

Or enable it only for one MCP server process:

```shellscript
coral --enable-episodes mcp-stdio
```

When enabled, agents should call `create_episode` once at the start of a task with an `intent`. Coral returns an `episode_id`; the agent must pass that ID as `episode_id` on follow-up Coral tool calls for the same task. A child task can pass `parent_episode_id` to `create_episode` to link back to the parent episode.

Episode records are stored locally today. Indexing and retrieval over those records are still in progress, so the first useful behavior is attribution: subsequent Coral MCP calls carry the episode id into Coral traces and query execution.
...
Feature-gated tools can also appear: `feedback` when the `feedback` feature is enabled, and `create_episode` when the `episodes` feature is enabled.
...
When `episodes` is enabled, `sql` also accepts optional `episode_id` for task attribution.

### Discovery tools

**`list_catalog`**: paginated list of database tables and parameterized table functions.

- Arguments: optional `schema`, `kind`, `limit`, `offset`. Omit `kind` (or pass `null`) to include both `"table"` and `"table_function"` items. When `episodes` is enabled, pass optional `episode_id` to attribute the call.
...
- Arguments: required `pattern` plus optional `schema`, `kind`, `ignore_case`, `limit`, `offset`. When `episodes` is enabled, pass optional `episode_id` to attribute the call.

**`describe_table`**: compact metadata for one table.

- Arguments: exact `schema` and `table`. When `episodes` is enabled, pass optional `episode_id` to attribute the call.
...
- Arguments: exact `schema` and `table`, plus optional `pattern`, `ignore_case`, `required_only`, `limit`, `offset`. When `episodes` is enabled, pass optional `episode_id` to attribute the call.

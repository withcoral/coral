...
| Tool     | `search`         | Find likely catalog items, columns, filters, observed values, native search paths, and provider search rows |
...
**`search`**: one-query discovery over Coral metadata, previously observed values, and provider-native search functions that explicitly opt in to Universal Search execution.

- Arguments: required `query` only.
- Returns typed results such as `catalog_item`, `column_hint`, `observed_value`, `native_search_path`, and `provider_search_result`, plus provider-status and truncation metadata.
- `provider_search_result` rows include the executed SQL call, source/function provenance, row ordinal, common id/title/url/snippet/score fields when available, and the full structured row payload.

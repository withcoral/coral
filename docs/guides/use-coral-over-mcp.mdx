...
**`search`**: one-query discovery over Coral metadata, previously observed values, and native search paths.
...
- Use it to turn a plain-text query, identifier, or phrase into candidate sources, SQL surfaces, columns, filters, or table-function calls.
...
- Use the returned `sql_reference` or `sql_call_example` with the `sql` tool when you need current source rows.

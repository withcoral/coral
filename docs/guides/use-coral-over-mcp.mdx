...
`result_get` may include `guidance` telling the agent to stop sequential
paging. Use the rows already seen and `row_count`; only rerun SQL when the
client can name a specific filter, aggregate, ordering, `LIMIT`, or narrower
projection that directly answers the question.

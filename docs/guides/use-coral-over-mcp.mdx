...
```json
{
  ...
  "row_count": 1200,
  "column_count": 8,
  "preview": {"offset": 0, "limit": 20, "has_more": true, "next_offset": 20, "rows": []},
...
The initial preview is byte-budgeted. If preview rows would make the `sql` response too large, Coral returns a metadata-only preview with `limit: 0`, `rows: []`, and `next_offset: 0`; call `result_get` with a column projection or rerun the SQL with filters, `LIMIT`, or smaller expressions.

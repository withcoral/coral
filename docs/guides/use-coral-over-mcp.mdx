...
`sql_codemode` is experimental and disabled by default. Enable it persistently with:
...
The tool executes an ordered `sql` array in one private query session and returns `results`, each with the original statement index, SQL text, and rows. SQL codemode accepts read-only query statements plus `CREATE TEMP TABLE ... AS SELECT` for session-local temporary tables; temporary-table creation statements do not produce result entries.

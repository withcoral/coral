...
The `sql` tool returns structured `{ columns, rows, row_count }` output, where `rows` is a JSON array of objects. To preserve exact values across JSON parsers, Coral encodes precision-sensitive numeric types as JSON **strings** rather than JSON numbers:

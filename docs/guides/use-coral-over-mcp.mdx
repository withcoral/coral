...
The `coral` system schema is part of that catalog. No-schema `list_catalog` and `coral://tables` include `coral.tables`, `coral.columns`, `coral.filters`, `coral.table_functions`, and `coral.inputs`; agents can also call `describe_table` and `list_columns` on those system tables.

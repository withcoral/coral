...
| Tool     | `search`         | Find likely catalog items, columns, filters, observed values, native search paths, and, when enabled, provider search rows |
...
**`search`**: one-query discovery over Coral metadata, previously observed values, and provider-native search paths. When the `search_provider_fanout` feature is enabled, it can also execute provider-native search functions that explicitly opt in to Universal Search execution.

- Arguments: required `query` only.
- Returns typed results such as `catalog_item`, `column_hint`, `observed_value`, and `native_search_path`, plus provider-status and truncation metadata.
- When `search_provider_fanout` is enabled, it can also return `provider_search_result` rows with the executed SQL call, source/function provenance, row ordinal, common id/title/url/snippet/score fields when available, and the full structured row payload.
- Use it to turn a plain-text query, identifier, or phrase into candidate sources, SQL surfaces, columns, filters, table-function calls, or executed provider-search rows.
- Provider-status metadata reports provider coverage and errors before final global ranking/truncation. `results_found` means a provider produced candidates, not that one of those candidates must appear in the returned `results` window.
- The MCP search tool is not paginated. If results are truncated or a provider reports matches without returned rows, retry with a more targeted keyword/identifier query instead of paging.
- Use the returned `sql_reference`, `sql_call_example`, or provider row payload with the `sql` tool when you need current source rows.
...
Universal Search only executes these provider-native functions automatically when `search_provider_fanout` is enabled and the source manifest explicitly opts that function in. Otherwise, `search` returns the table-function path and agents should call it explicitly through `sql`.
...
| Tool     | `sql`            | Execute read-only SQL against the Coral database        |
| Tool     | `list_catalog`   | List database tables and table functions with pagination |
| Tool     | `search`         | Find likely catalog items, columns, filters, observed values, and native search paths |
| Tool     | `describe_table` | Show compact metadata for one database table            |
| Tool     | `list_columns`   | List columns for one database table with pagination     |
| Resource | `coral://guide`  | Database workflow guide for agents                     |
| Resource | `coral://tables` | JSON summaries of query-visible database tables, including Coral catalog tables and required filters |
...
**`search`**: one-query clue routing over Coral discovery metadata and local search indexes.

- Arguments: required `query` only.
- Returns typed hints such as `catalog_item`, `column_hint`, `observed_value`, and `native_search_path`, plus provider-status and truncation metadata.
- When `search_provider_fanout` is enabled, it can also execute opted-in provider-native search functions and return `provider_search_result` rows.

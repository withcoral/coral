...
Recipes that publish both `table_function` and `mcp_tool` targets appear as additional MCP tools. Calling one of those tools executes the recipe through its published SQL table function, so it returns the same rows as `coral sql "select * from recipes.<name>(...)"`.

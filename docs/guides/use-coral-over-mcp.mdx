...
- Searches table and table-function names, descriptions, ordinary table columns, arguments, result columns, guides, and required filters.
- Ordinary table hits include capped `column_preview` entries plus `column_count` and `omitted_column_count`. Use `list_columns` when you need the full schema.

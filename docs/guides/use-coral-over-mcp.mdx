...
Benchmark episode runs can also expose an experimental `search_trajectory` tool. It searches local trajectory memory for an exact-intent golden SQL path and returns procedure steps only, not cached answer rows.

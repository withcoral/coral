...
When an agent needs to write SQL, have it prefer the SQL-ready identifier fields
from `coral.tables` and `coral.columns` such as `sql_table_ref`,
`sql_column_ref`, and `sql_qualified_column_ref`. Those fields are safer to
copy verbatim than raw `table_name` or `column_name` values when identifiers
use mixed case or other characters that require quoting.
